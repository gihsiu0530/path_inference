...
def parse_args():
    ...
    parser.add_argument(
        "--no-depth",
        action="store_true",
        help="沒有深度資料時關閉：不讀取/不複製 depth，仍輸出 depth_timestep 佔位（= img_timestep）。",
    )
...
def prepare_output_dir(path, overwrite, use_depth=True):
    if path.exists():
        if not overwrite:
    ...
    (path / "seg").mkdir()
    # 深度關閉時不建立 depth 輸出資料夾
    if use_depth:
        (path / "depth").mkdir()


def resample_video(video_dir, output_dir, interval_ns, overwrite, use_depth=True):
    prepare_output_dir(output_dir, overwrite, use_depth)

    img_items = image_timestamps(video_dir / "img")
    seg_items = image_timestamps(video_dir / "seg")
    # 深度關閉時不讀 depth 影像（可完全沒有 depth 資料夾）
    depth_items = image_timestamps(video_dir / "depth") if use_depth else None
    odom_rows = read_csv_rows(video_dir / "odom.csv", "timestep", "odom")
    # poseimu 來源改用 odom.csv：下游載入器並未使用 poseimu，故不再另外
    # 讀取 / 輸出 poseimu 檔，poseimu 時間戳直接沿用 odom 的 timestep。

    start_ts = max(
        img_items[0][0],
        seg_items[0][0],
        *([depth_items[0][0]] if use_depth else []),
        int(odom_rows[0]["timestep"]),
    )
    end_ts = min(
        img_items[-1][0],
        seg_items[-1][0],
        *([depth_items[-1][0]] if use_depth else []),
    ...
    with (output_dir / "odom.csv").open("w", newline="", encoding=CSV_ENCODING) as odom_file, (
        ...
        while target <= end_ts:
            ...
            # 深度關閉時 depth_timestep 以 img_ts 佔位，且不複製 depth 影像
            if use_depth:
                depth_ts, depth_path = nearest_by_timestamp(depth_items, img_ts)
            else:
                depth_ts, depth_path = img_ts, None
            odom_row = nearest_csv_row(odom_rows, "timestep", target)
            odom_ts = int(odom_row["timestep"])
            # poseimu 時間戳沿用 odom（維持 resample_index 欄位相容）。
            poseimu_ts = odom_ts

            key = (img_ts, seg_ts, depth_ts, odom_ts)
...
def main():
    ...
    for index in range(args.start, args.end + 1):
        ...
        count = resample_video(
            video_dir, output_dir, args.interval_ns, args.overwrite, use_depth=not args.no_depth
        )
