...
def _resolve_runtime_local_path(value, label):
    """Resolve a runtime asset and reject paths outside real_time_ff."""
    path = pathlib.Path(str(value)).expanduser()
    if not path.is_absolute():
        path = pathlib.Path(_REPO_ROOT) / path
    path = path.resolve()
    runtime_root = pathlib.Path(_REPO_ROOT).resolve()
    try:
        path.relative_to(runtime_root)
    except ValueError as exc:
        raise ValueError(f"{label} must stay under {runtime_root}: {path}") from exc
    return str(path)


def _resolve_depth_checkpoint(value):
    """Allow exactly the configured external Depth-Anything weight file."""
    path = pathlib.Path(str(value)).expanduser().resolve()
    expected = pathlib.Path(DEFAULT_DA_V2_CKPT).expanduser().resolve()
    if path != expected:
        raise ValueError(f"~da_v2_ckpt must be {expected}: got {path}")
    return str(path)
...
class RealtimePlannerNode:
    def __init__(self):
        ...
        self.in_topic = rospy.get_param(
            "~in_topic", "/zed2i/zed_node/left/image_rect_color")
        ...
        self.checkpoint = _resolve_runtime_local_path(
            rospy.get_param("~checkpoint", DEFAULT_CHECKPOINT), "~checkpoint"
        )
        self.sample_interval = float(rospy.get_param("~sample_interval", SAMPLE_INTERVAL))
        # One three-way plotting switch. If ~plot_mode is omitted, preserve the
        # old ~save_plots boolean: true maps to realtime and false maps to off.
        legacy_save_plots = bool(rospy.get_param("~save_plots", True))
        default_plot_mode = "realtime" if legacy_save_plots else "off"
        self.plot_mode = str(
            rospy.get_param("~plot_mode", default_plot_mode)
        ).strip().lower()
        if self.plot_mode not in {"realtime", "with_gt", "off"}:
            raise ValueError("~plot_mode must be realtime, with_gt, or off")
        self.save_plots = self.plot_mode != "off"
        ...
        self.da_v2_repo = _resolve_runtime_local_path(
            rospy.get_param("~da_v2_repo", DEFAULT_DA_V2_REPO), "~da_v2_repo"
        )
        self.da_v2_ckpt = _resolve_depth_checkpoint(
            rospy.get_param("~da_v2_ckpt", DEFAULT_DA_V2_CKPT)
        )
        ...
        self.processor = SegformerImageProcessor.from_pretrained(
            SEGFORMER_NAME, local_files_only=True
        )
        ...
        self._plot_dir = None
        self._plot_seq = 0
        # Used only by with_gt mode to hold predictions until six future poses
        # (3 seconds at the default cadence) have arrived.
        self._pending = deque()
        ...
        if self.plot_mode != "off":
            ...
            if self.plot_mode == "realtime":
                behavior = "saved immediately without GT; lateral view=-1..+1 m"
            else:
                behavior = (
                    f"saved after {N_FUTURE_FRAMES * self.sample_interval:.1f}s "
                    "with driven-path GT"
                )
            rospy.loginfo(
                f"[planner] plot_mode={self.plot_mode}: imgs/<ts>/inference_plots/ "
                f"[{layout}] ({behavior}; _plot_seg:=false to drop seg panels)"
            )
        else:
            rospy.loginfo("[planner] plot_mode=off: inference plots disabled")
    ...
    def cb_image(self, msg: RosImage):
        ...
        if self.last_sample_time is not None and now < self.last_sample_time:
            ...
            self.buffer = RealtimeSequenceBuffer(self.use_depth)
            self.last_sample_time = None
            # Preserve incomplete with_gt plots before starting a new bag loop.
            self._flush_pending(force=True)
            # Treat a clock restart as a new run: start a fresh plot folder so
            # each bag replay lands in its own imgs/<ts>/ dir.
            self._plot_dir = None
            self._plot_seq = 0
    ...
    def process(self, msg: RosImage):
        ...
        # In with_gt mode this pose is the next driven waypoint for predictions
        # made on earlier samples. Realtime/off modes return immediately.
        ...
        self._handle_plot(final_traj, msg.header.stamp, pose)
    ...
    def _ensure_plot_dir(self) -> pathlib.Path:
        """
        imgs/<MM_DD_HH_MM_SS>/inference_plots — mirrors the offline
        park_L2_ASAP.mk_save_dir layout, rooted under real_time_ff/. Created
        ...
        if self._plot_dir is None:
            ...
            self._plot_dir = (pathlib.Path(_REPO_ROOT) / "imgs"
    ...
    def _handle_plot(self, final_traj: np.ndarray, stamp, pose: np.ndarray) -> None:
        """
        Save immediately, queue for driven-path GT, or do nothing by plot_mode.

        The trajectory arrives in the plot's (x_left, y_front) convention;
        plan() already applied the one required lateral sign flip.
        """
        if self.plot_mode == "off":
            return
    ...
    def _advance_pending(self, pose: np.ndarray) -> None:
        """Append one driven waypoint and write completed with_gt records."""
        if self.plot_mode != "with_gt":
            return
    ...
    def _flush_pending(self, force: bool = False) -> None:
        """Write completed or, when forced, partially completed with_gt records."""
        while self._pending:
    ...
    def _write_plot(self, rec: dict) -> None:
        """
        Write the common panels and optionally add retrospective driven-path GT.
        """
