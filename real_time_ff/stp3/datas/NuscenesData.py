...
def build_depth_path(dataroot_depths: str, camera_sample, cam_name: str) -> str:
    """
    產生預處理深度 .npy 路徑：
    <DEPTH_ROOT>/<CAM_NAME>/npy/<同名>.npy
    """
    filename = os.path.basename(camera_sample['filename']).split('.')[0] + '.npy'
    return os.path.join(dataroot_depths, cam_name, filename)

def get_preprocessed_depths_for_rec(nusc, rec, cams, depth_root) -> torch.Tensor:
    """
    對單一 timestamp 的多相機，載入已預處理(224x224)深度，回傳 shape: (1, N, 1, 224, 224)
    不做任何 resize/crop/round。
    """
    depth_tensors = []
    for cam in cams:
        cam_sd = nusc.get('sample_data', rec['data'][cam])
        depth_path = build_depth_path(depth_root, cam_sd, cam)
        depth_np = np.load(depth_path)
        if depth_np.dtype != np.float32:
            depth_np = depth_np.astype(np.float32)     # 訓練前轉回 FP32 比較穩定
        depth = torch.from_numpy(depth_np).unsqueeze(0).unsqueeze(0)  # (1,1,224,224)
        depth_tensors.append(depth)
    # 拼成 (1, N, 1, H, W)
    return torch.stack(depth_tensors, dim=1)
...
class FuturePredictionDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, nusc, is_train, cfg):
        ...
        # NuscenesData.py 的 __init__ 最後（indices/ixes 都準備好之後）
        self.commands_per_index = []
        for seq in self.indices:
            index_t = seq[self.cfg.TIME_RECEPTIVE_FIELD - 1]
            rec = self.ixes[index_t]
            _, cmd = self.get_gt_trajectory(rec, index_t)
            self.commands_per_index.append(cmd)
    ...
    def get_input_data(self, rec):
        ...
        for cam in cameras:
            ...
            if self.cfg.LIFT.GT_DEPTH:
                # 1) 用 DEPTH_ROOT，而不是 dataroot/depths（因為你存在 trainval/depths）
                depth_root = getattr(self.cfg.LIFT, "DEPTH_ROOT", None)
                assert depth_root is not None, "請在 cfg.LIFT.DEPTH_ROOT 指定深度根目錄"

                # 2) 組出 .npy 路徑（<DEPTH_ROOT>/<CAM_NAME>/npy/<同名>.npy）
                depth_file_name = build_depth_path(depth_root, camera_sample, cam)

                # 3) 讀檔 + dtype 處理（你離線多半是 float16；轉回 float32 比較穩定）
                depth_np = np.load(depth_file_name)  # shape (224,224) for preprocessed
                if depth_np.dtype != np.float32:
                    depth_np = depth_np.astype(np.float32)

                # 4) 依旗標：預處理過就「直接用」；否則走舊路徑（resize+crop）
                if getattr(self.cfg.LIFT, "DEPTH_PREPROCESSED", False):
                    # 這裡的深度已經跟影像在「幾何上對齊」（224×224），不再做 interpolate + crop
                    depth = torch.from_numpy(depth_np)                 # (H, W)
                    depths.append(depth.unsqueeze(0).unsqueeze(0))     # (1,1,H,W)
                else:
                    # 舊路徑（如果你之後要支援原尺寸離線檔，就走這條）
                    depth = torch.from_numpy(depth_np).unsqueeze(0).unsqueeze(0)  # (1,1,H0,W0)
                    depth = F.interpolate(depth, scale_factor=self.cfg.IMAGE.RESIZE_SCALE, mode='bilinear', align_corners=False)
                    depth = depth.squeeze(0).squeeze(0)  # (H1,W1)
                    crop = self.augmentation_parameters['crop']
                    depth = depth[crop[1]:crop[3], crop[0]:crop[2]]    # (Hfinal, Wfinal)
    ...
    def get_egomotion_between(self, rec_t0, rec_t1):
        # Identity (4x4) -> 6DoF 最後會變成全 0
        ...
        if rec_t0['scene_token'] == rec_t1['scene_token']:
            ...
            # t0 -> t1
            T = invert_matrix_egopose_numpy(egopose_t1).dot(egopose_t0)
            T[3, :3] = 0.0
            T[3, 3] = 1.0

        T = torch.tensor(T).float()
        vec6 = mat2pose_vec(T)          # (6,)
        return vec6.unsqueeze(0)        # (1,6)
    ...
    def get_future_egomotion(self, rec, index):
        ...
        # print("future_egomotion.unsqueeze(0)",future_egomotion.unsqueeze(0))
    ...
    def __getitem__(self, index):
        ...
        seq = self.indices[index]
        T_rf = self.receptive_field
        cur_i = T_rf - 1  # current 的位置

        # 先把 future_egomotion 算好（只算一次）
        future_egomotion_list = []
        for t in range(len(seq)):
            if t < cur_i:
                rec_t0 = self.ixes[seq[t]]
                rec_t1 = self.ixes[seq[t + 1]]          # 仍在 past/receptive_field 內
                ego = self.get_egomotion_between(rec_t0, rec_t1)
            else:
                ego = torch.zeros(1, 6, dtype=torch.float32)  # current 以及之後全 0（不看未來）
            future_egomotion_list.append(ego)

        data['future_egomotion'] = future_egomotion_list
        ...
        for i, index_t in enumerate(self.indices[index]):
            ...
            if i < self.receptive_field and self.cfg.LIFT.GT_DEPTH:
                depths = get_preprocessed_depths_for_rec(self.nusc, rec, cams, depth_root)
                # (1, N, 1, 224, 224)
                data['depths'].append(depths)
            
            # raise RuntimeError("現在版本應有深度資訊")
            ...
            if i == self.cfg.TIME_RECEPTIVE_FIELD-1:
                ...
                data['sample_trajectory'] = torch.empty(0, dtype=torch.float32)
        ...
        for key, value in data.items():
            if key in ['segmentation', 'instance', 'future_egomotion', 'pedestrian']:
                data[key] = torch.cat(value, dim=0)
            elif key == 'depths' and len(value) > 0:
                data[key] = torch.cat(value, dim=0)  # (T_rf, N, 1, 224, 224)
        ...
        # front_img_path = os.path.join(self.dataroot, cam_sample['filename'])

        # # seg2d .npy 路徑（你原本就有）
        # seg2d_root = self.cfg.SEG2D_ROOT
        # seg2d_path = build_seg2d_path(front_img_path, seg2d_root)

        # # === 新增：組 224 影像路徑並讀檔 ===
        # # RGB 224：把 /samples/CAM_FRONT/ 換成 /samples_224*224/CAM_FRONT/
        # rgb224_path = front_img_path.replace(
        #     "/samples/CAM_FRONT/", "/samples_224*224/CAM_FRONT/"
        # )

        # # Seg 224：把 /seg2d/xxx.npy 換到 /seg_cl4_png/xxx_cls4_224.png
        # seg224_path = seg2d_path.replace("/seg2d/", "/seg_cl4_png/").replace(".npy", "_cls4_224.png")

        # # 讀檔 → 轉 RGB → 存成 uint8 張量（H,W,3）
        # rgb224 = cv2.cvtColor(cv2.imread(rgb224_path, cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
        # seg224 = cv2.cvtColor(cv2.imread(seg224_path, cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
        # data['rgb_224'] = torch.from_numpy(rgb224).to(torch.uint8)
        # data['seg_224'] = torch.from_numpy(seg224).to(torch.uint8)
