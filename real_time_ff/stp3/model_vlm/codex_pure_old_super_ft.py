...
class FastTrajectoryPlanner(nn.Module):
    ...
    def __init__(self, cfg, n_future: int, input_size: int = 224):
        ...
        self.ped_traj_feat_dim = int(getattr(cfg, "PED_TRAJ_FEAT_DIM", 12))
        self.max_ped_agents = int(getattr(cfg, "PED_MAX_AGENTS", 64))
        self.ped_frames = int(getattr(cfg, "PED_INPUT_FRAMES", 9))
        self.ped_gate_min = float(getattr(cfg, "PED_GATE_MIN", 0.10))
        self.ped_gate_init_bias = float(getattr(cfg, "PED_GATE_INIT_BIAS", 0.0))
        self.ped_start_step = int(getattr(cfg, "PED_START_STEP", 1))
        self.ped_ramp_steepness = float(getattr(cfg, "PED_RAMP_STEEPNESS", 2.0))
        self.ped_bev_gate_min = float(getattr(cfg, "PED_BEV_GATE_MIN", 0.05))
        self.ped_spatial_scale = float(getattr(cfg, "PED_SPATIAL_SCALE", 0.5))
        self.ped_bev_scale = float(getattr(cfg, "PED_BEV_SCALE", 1.0))
        self.ped_use_goal = bool(getattr(cfg, "PED_USE_GOAL", True))
        self.ped_step_proj = nn.Sequential(
            nn.Linear(self.ped_traj_feat_dim + 1, c),
            nn.GELU(),
            nn.Linear(c, c),
        )
        ...
        self.ped_time_pe = nn.Parameter(torch.randn(self.ped_frames, c) * 0.02)
        self.ped_agent_embed = nn.Embedding(self.max_ped_agents, c)
    ...
    def _prepare_ped_traj_inputs(
        ...
        b, m, t, feat_dim = ped.shape
        if m > self.max_ped_agents:
            ped = ped[:, :self.max_ped_agents]
            m = ped.shape[1]
        if feat_dim < self.ped_traj_feat_dim:
            pad = torch.zeros(b, m, t, self.ped_traj_feat_dim - feat_dim, device=device, dtype=ped.dtype)
            ped = torch.cat([ped, pad], dim=-1)
        elif feat_dim > self.ped_traj_feat_dim:
    ...
    def encode_ped_traj(
        ...
        ped, ped_mask, step_mask = self._prepare_ped_traj_inputs(
        ...
        agent_ids = torch.arange(m, device=device).clamp(max=self.max_ped_agents - 1)
        ...
        ped_max = ped_tokens.masked_fill(~step_mask[:, :, :, None], neg_inf).amax(dim=(1, 2))
    ...
    def forward(
        ...
        ped_tokens, ped_ctx = self.encode_ped_traj(
            ped_traj_preds, ped_traj_mask, ped_traj_valid_steps, device
        )
        if ped_bev_map is not None:
            ...
            # fused_mid / fused are flattened to (B*T_rf, C, H, W), so repeat
            # the present-frame pedestrian BEV prior across the receptive-field
            # frames before fusing.
            ...
            ped_mid_feat = self.ped_bev_align_head(self.ped_bev_encoder(ped_bev_mid))
            ped_deep_feat = self.ped_bev_align_head(self.ped_bev_encoder(ped_bev_deep))
            if ped_mid_feat.shape[-2:] != fused_mid.shape[-2:]:
                ped_mid_feat = F.interpolate(
                    ped_mid_feat, size=fused_mid.shape[-2:], mode="bilinear", align_corners=False
                )
            if ped_deep_feat.shape[-2:] != fused.shape[-2:]:
                ped_deep_feat = F.interpolate(
                    ped_deep_feat, size=fused.shape[-2:], mode="bilinear", align_corners=False
                )
            ...
            ped_mid_gate = self.ped_bev_gate_min + (1.0 - self.ped_bev_gate_min) * ped_mid_gate
            ped_deep_gate = self.ped_bev_gate_min + (1.0 - self.ped_bev_gate_min) * ped_deep_gate
            ...
            self.last_ped_bev_gate = 0.5 * (
                ped_mid_gate.mean(dim=(1, 2, 3)) + ped_deep_gate.mean(dim=(1, 2, 3))
            ).detach()
        else:
        ...
        if ped_tokens is not None:
            spatial_tokens = torch.cat([spatial_tokens, self.ped_spatial_scale * ped_tokens], dim=1)
        ...
        if ped_ctx is not None:
            ctx = ctx + self.ped_spatial_scale * ped_ctx
        ...
        if ped_ctx is not None:
            ped_ctx_expand = ped_ctx.unsqueeze(1).expand(-1, self.n_future, -1)
            ped_gate_raw = self.ped_fuse_gate(torch.cat([queries, ped_ctx_expand], dim=-1))
            step_idx = torch.arange(self.n_future, device=device, dtype=queries.dtype)
            center = float(min(max(self.ped_start_step, 0), max(self.n_future - 1, 0)))
            if self.n_future <= 1:
                ped_active = torch.ones(b, self.n_future, 1, device=device, dtype=queries.dtype)
            else:
                steepness = max(self.ped_ramp_steepness, 1e-3)
                ped_active_1d = torch.sigmoid((step_idx - center) / steepness)
                ped_active = ped_active_1d.view(1, self.n_future, 1).expand(b, -1, -1)
            ped_gate = (self.ped_gate_min + (1.0 - self.ped_gate_min) * ped_gate_raw) * ped_active
            queries = queries + ped_gate * ped_ctx_expand
    ...
    def planning(
        ...
        *,
        ...
        if ped_bev_points is not None and ped_bev_valid_steps is not None:
            ped_repulse_t = self.dynamic_repulsion_loss(
                pred_xy, ped_bev_points, ped_bev_valid_steps, return_per_t=True
            )
            ped_repulse = self.dynamic_repulsion_loss(
