...
class PedRiskTrajectoryAdapter(nn.Module):
    """Apply a hard-gated trajectory correction using predicted pedestrian geometry."""

    def __init__(self, cfg, n_future: int):
        super().__init__()
        self.n_future = int(n_future)
        self.safe_dist = float(getattr(cfg, "PED_RISK_GATE_SAFE_DIST", 5.0))
        self.max_delta = float(getattr(cfg, "PED_RISK_ADAPTER_MAX_DELTA", 4.0))
        hidden_dim = int(getattr(cfg, "PED_RISK_ADAPTER_HIDDEN_DIM", 128))

        # Per step: baseline xy, baseline velocity, nearest-ped relative xy,
        # normalized distance/proximity, valid flag, and normalized time.
        ...
        # A newly added adapter must initially preserve the loaded baseline.
        nn.init.zeros_(self.delta_head[-1].weight)
        nn.init.zeros_(self.delta_head[-1].bias)

        self.last_has_ped = None
        self.last_risk_gate = None
        self.last_predicted_min_distance = None
        self.last_delta_xy = None

    def _zero_stats(self, baseline_xy: torch.Tensor):
        b = baseline_xy.shape[0]
        device = baseline_xy.device
        dtype = baseline_xy.dtype
        self.last_has_ped = torch.zeros(b, device=device, dtype=torch.bool)
        self.last_risk_gate = torch.zeros(b, device=device, dtype=torch.bool)
        self.last_predicted_min_distance = torch.full(
            (b,), float("inf"), device=device, dtype=dtype
        )
        self.last_delta_xy = torch.zeros_like(baseline_xy)

    def _unchanged_with_adapter_graph(self, baseline_xy: torch.Tensor):
        # Preserve exact values while keeping a zero-valued adapter graph so an
        # all-no-pedestrian training batch can still run backward safely.
        zero = self.delta_head[-1].weight.sum() * 0.0
        return baseline_xy + zero

    def forward(self, baseline_xy, ped_bev_points=None, ped_bev_valid_steps=None):
        baseline_xy = baseline_xy.detach()
        if ped_bev_points is None or ped_bev_valid_steps is None:
            self._zero_stats(baseline_xy)
            return self._unchanged_with_adapter_graph(baseline_xy)
        ...
        ped_xy = ped_bev_points.to(
            device=baseline_xy.device, dtype=baseline_xy.dtype, non_blocking=True
        )
        ped_valid = ped_bev_valid_steps.to(
            device=baseline_xy.device, dtype=torch.bool, non_blocking=True
        )
        if ped_xy.dim() != 4 or ped_valid.dim() != 3:
            raise ValueError(
                "ped_bev_points should be (B,M,T,2) and "
                "ped_bev_valid_steps should be (B,M,T)"
            )

        t = min(baseline_xy.shape[1], ped_xy.shape[2], ped_valid.shape[2])
        if t <= 0:
            self._zero_stats(baseline_xy)
            return self._unchanged_with_adapter_graph(baseline_xy)
        ped_xy = ped_xy[:, :, :t]
        ped_valid = ped_valid[:, :, :t]
        baseline_t = baseline_xy[:, :t]

        dist = torch.norm(baseline_t[:, None] - ped_xy, dim=-1)
        inf = torch.full_like(dist, float("inf"))
        valid_dist = torch.where(ped_valid, dist, inf)
        min_dist_t, nearest_idx = valid_dist.min(dim=1)
        valid_t = ped_valid.any(dim=1)
        has_ped = valid_t.any(dim=1)
        predicted_min_distance = min_dist_t.min(dim=1).values
        risk_gate = has_ped & (predicted_min_distance < self.safe_dist)

        gather_idx = nearest_idx[:, None, :, None].expand(-1, 1, -1, 2)
        nearest_xy = ped_xy.gather(dim=1, index=gather_idx).squeeze(1)
        nearest_xy = torch.where(valid_t[..., None], nearest_xy, baseline_t)
        relative_xy = nearest_xy - baseline_t

        baseline_vel = torch.cat(
            [baseline_t[:, :1], baseline_t[:, 1:] - baseline_t[:, :-1]], dim=1
        )
        safe_dist = max(self.safe_dist, 1e-3)
        finite_min_dist_t = torch.where(
            valid_t, min_dist_t, torch.full_like(min_dist_t, safe_dist * 2.0)
        )
        distance_norm = (finite_min_dist_t / safe_dist).clamp(0.0, 2.0)
        proximity = (1.0 - distance_norm).clamp(0.0, 1.0)
        time = torch.linspace(
            0.0, 1.0, t, device=baseline_xy.device, dtype=baseline_xy.dtype
        ).view(1, t, 1).expand(baseline_xy.shape[0], -1, -1)
        features = torch.cat(
            [
                baseline_t / 30.0,
                baseline_vel / 10.0,
                relative_xy / safe_dist,
                distance_norm[..., None],
                proximity[..., None],
                valid_t.to(baseline_xy.dtype)[..., None],
                time,
            ],
            dim=-1,
        )
        encoded = self.step_encoder(features)
        encoded, _ = self.temporal(encoded)
        raw_delta = torch.tanh(self.delta_head(encoded)) * self.max_delta

        delta = torch.zeros_like(baseline_xy)
        delta[:, :t] = raw_delta * risk_gate.to(baseline_xy.dtype).view(-1, 1, 1)
        final_xy = baseline_xy + delta

        self.last_has_ped = has_ped.detach()
        self.last_risk_gate = risk_gate.detach()
        self.last_predicted_min_distance = predicted_min_distance.detach()
        self.last_delta_xy = delta.detach()
        return final_xy
...
class FastTrajectoryPlanner(nn.Module):
    ...
    def __init__(self, cfg, n_future: int, input_size: int = 224):
        ...
        self.ped_risk_adapter = PedRiskTrajectoryAdapter(cfg, self.n_future)
    ...
    def forward(self, rgb_seq: torch.Tensor, seg_rgb_seq: torch.Tensor, seg_id_seq: torch.Tensor,
        ...
        if ped_tokens is not None:
            ...
            queries, ped_gate = self.ped_query_fusion(
                queries,
                ped_tokens.to(queries.dtype),
                ped_key_padding_mask,
                ped_ctx.to(queries.dtype),
                active_gate,
            )
...
class VLM_STP3_Gen(nn.Module):
    def __init__(self, cfg):
        ...
        print("model : codex_pure_fast_ASAP_admlp_coarse_super_ft")
    ...
    def _print_planning_loss_weights(self):
        ...
        lam_ped_repulse = float(getattr(self.cfg, "LOSS_PED_REPULSE_W", 0.0))
        ...
        print(f"  lam_ped_repulse = {lam_ped_repulse}")
    ...
    def forward(self, image, intrinsics, extrinsics, future_egomotion, *,
                ...
                ego_history_egomotion=None, admlp_input=None,
    ...
    def planning(
        ...
        *,
        ...
        # The shared planner always runs without pedestrian inputs. Its output is
        # the immutable baseline used by both the risk gate and final residual.
        baseline_pred = self.vlm(
            self._last_rgb_seq,
            self._last_seg_seq,
        ...
        final_xy = self.vlm.ped_risk_adapter(
            baseline_pred[..., :2],
            ped_bev_points=ped_bev_points,
            ped_bev_valid_steps=ped_bev_valid_steps,
        )
        pred = torch.cat([final_xy, baseline_pred[..., 2:]], dim=-1)
        final_traj = pred
        ...
        ped_risk_adapter = self.vlm.ped_risk_adapter
        risk_gate = ped_risk_adapter.last_risk_gate
        has_ped = ped_risk_adapter.last_has_ped
        predicted_min_distance = ped_risk_adapter.last_predicted_min_distance
        adapter_delta = ped_risk_adapter.last_delta_xy
        if risk_gate is not None:
            loss_dict["ped_risk_gate_ratio"] = risk_gate.float().mean()
        if has_ped is not None:
            loss_dict["ped_adapter_has_ped_ratio"] = has_ped.float().mean()
        if predicted_min_distance is not None:
            finite_distance = predicted_min_distance[torch.isfinite(predicted_min_distance)]
            loss_dict["ped_predicted_min_distance"] = (
                finite_distance.mean()
                if finite_distance.numel() > 0
                else torch.tensor(0.0, device=device)
            )
        if adapter_delta is not None:
            delta_norm = torch.norm(adapter_delta, dim=-1)
            loss_dict["ped_adapter_delta_mean"] = delta_norm.mean()
            loss_dict["ped_adapter_delta_max"] = delta_norm.max()
        ped_onoff_delta_mean = torch.tensor(0.0, device=device)
        ped_onoff_delta_final = torch.tensor(0.0, device=device)
        ped_onoff_delta_max = torch.tensor(0.0, device=device)
        ped_onoff_has_signal = torch.tensor(0.0, device=device)
        if bool(getattr(self.cfg, "PED_ON_OFF_DELTA_LOG", True)):
            has_ped_signal = False
            if torch.is_tensor(self._last_ped_traj_mask):
                has_ped_signal = has_ped_signal or bool(self._last_ped_traj_mask.detach().bool().any().item())
            if torch.is_tensor(self._last_ped_bev_map):
                has_ped_signal = has_ped_signal or bool((self._last_ped_bev_map.detach().float().amax() > 0).item())
            if torch.is_tensor(ped_bev_valid_steps):
                has_ped_signal = has_ped_signal or bool(ped_bev_valid_steps.detach().bool().any().item())

            if has_ped_signal:
                ped_onoff_has_signal = torch.tensor(1.0, device=device)
                last_state_names = [
                    "last_coarse_xy",
                    "last_endpoint_xy",
                    "last_residual_gate",
                    "last_residual_gate_raw",
                    "last_residual_xy_raw",
                    "last_residual_xy_gated",
                    "last_ped_gate",
                    "last_ped_bev_gate",
                ]
                last_state = {name: getattr(self.vlm, name, None) for name in last_state_names}
                try:
                    with torch.no_grad():
                        pred_no_ped = self.vlm(
                            self._last_rgb_seq,
                            self._last_seg_seq,
                            self._last_seg_id_seq,
                            self._last_depth_seq,
                        ...
                        ped_onoff_delta = torch.norm(pred_xy.detach() - pred_no_ped[..., :2], dim=-1)
                        ped_onoff_delta_mean = ped_onoff_delta.mean()
                        ped_onoff_delta_final = ped_onoff_delta[:, -1].mean()
                        ped_onoff_delta_max = ped_onoff_delta.max()
                finally:
                    for name, value in last_state.items():
                        setattr(self.vlm, name, value)
        loss_dict["ped_onoff_has_signal"] = ped_onoff_has_signal
        loss_dict["ped_onoff_traj_delta_mean"] = ped_onoff_delta_mean
        loss_dict["ped_onoff_traj_delta_final"] = ped_onoff_delta_final
        loss_dict["ped_onoff_traj_delta_max"] = ped_onoff_delta_max
