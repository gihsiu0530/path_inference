...
class VLM_Generative(BaseVLM_Generative):
    ...
    預設 F=8，對應你的 CSV 欄位：
    [pred_bbox_cx, pred_bbox_cy, pred_bbox_w, pred_bbox_h,
     pred_goal_cx, pred_goal_cy, pred_goal_w, pred_goal_h]

    目前這個分支只做 learned fusion，不假設已經是 BEV 座標；
    因此之後 dataset 對齊 CSV 時，只要把對應 tensor 餵進來即可。
    """
    ...
    def __init__(self, clip_name: str, device: torch.device, n_future: int, input_size: int = 224, prompts=None):
        ...
        self.ped_traj_feat_dim = int(getattr(self, 'PED_TRAJ_FEAT_DIM', 8))
        ...
        self.ped_time_pe = nn.Parameter(torch.randn(self.n_future, self.vis_width) * 0.02)
        ...
        self.h0_proj = nn.Sequential(
            nn.Linear(self.vis_width * 3, self.vis_width),
            nn.GELU(),
            nn.Linear(self.vis_width, self.vis_width),
        )

        # gate 下限與初始化
        self.ped_gate_min = float(getattr(self, 'PED_GATE_MIN', 0.1))
        self.ped_gate_init_bias = float(getattr(self, 'PED_GATE_INIT_BIAS', 1.5))

        # ped gate 最後一層 bias 初始化成正值
        nn.init.constant_(self.ped_fuse_gate[2].bias, self.ped_gate_init_bias)

        # 給 logging 用
        self.last_ped_gate = None
        self.last_ped_gate_high_risk = None
        self.last_ped_fused_norm = None
        self.last_fused_norm = None
        self.last_ped_to_vis_norm_ratio = None
        self.last_ped_high_risk_mask = None
    ...
    def _prepare_ped_traj_inputs(
        ...
        if T < self.n_future:
            pad = torch.zeros(B, M, self.n_future - T, ped.shape[-1], device=device, dtype=ped.dtype)
            ped = torch.cat([ped, pad], dim=2)
        elif T > self.n_future:
            ped = ped[:, :, :self.n_future]
    ...
    def generate_autoregressive(
        ...
        *,
        ...
        if ped_tokens is not None:
            ...
            self.last_ped_fused_norm = ped_fused.norm(dim=-1).mean().detach()
            self.last_fused_norm = fused_vis_only.norm(dim=-1).mean().detach()
            self.last_ped_to_vis_norm_ratio = (
                self.last_ped_fused_norm / self.last_fused_norm.clamp(min=1e-6)
            ).detach()

            high_risk_mask = None
            ped_in, ped_mask, step_mask = self._prepare_ped_traj_inputs(
                ped_traj_preds, ped_traj_mask, ped_traj_valid_steps, device
            )
            if ped_in is not None and ped_in.size(-1) >= 2:
                ped_xy = ped_in[..., :2]
                ped_dist = torch.norm(ped_xy, dim=-1)
                valid_mask = step_mask if step_mask is not None else torch.ones_like(ped_dist, dtype=torch.bool)
                ped_dist = torch.where(valid_mask, ped_dist, torch.full_like(ped_dist, 1e6))

                risk_radius = float(getattr(self, 'PED_HIGH_RISK_RADIUS', 3.0))
                high_risk_mask = (ped_dist.min(dim=2).values.min(dim=1).values < risk_radius)

            if high_risk_mask is None:
                high_risk_mask = torch.zeros(B, dtype=torch.bool, device=device)

            self.last_ped_high_risk_mask = high_risk_mask.detach()
            if high_risk_mask.any():
                self.last_ped_gate_high_risk = ped_gate.squeeze(-1)[high_risk_mask].mean().detach()
            else:
                self.last_ped_gate_high_risk = torch.tensor(0.0, device=device)
        else:
            self.last_ped_gate = torch.zeros(B, T, device=device)
            self.last_ped_fused_norm = torch.tensor(0.0, device=device)
            self.last_fused_norm = fused_vis_only.norm(dim=-1).mean().detach()
            self.last_ped_to_vis_norm_ratio = torch.tensor(0.0, device=device)
            self.last_ped_gate_high_risk = torch.tensor(0.0, device=device)
            self.last_ped_high_risk_mask = torch.zeros(B, dtype=torch.bool, device=device)

        global_ctx = fused.mean(dim=1)
        h = self.h0_proj(torch.cat([global_ctx, text_vis, ped_ctx], dim=-1))
