...
class VLM_Generative(nn.Module):
    ...
    def make_coarse_baseline(self, ego_seq: torch.Tensor, T: int = None) -> torch.Tensor:
        """
        history-aware coarse baseline：
        - 不看影像，但不再只用最近一步。
        - 改用整段 ego 歷史做帶權擬合，估計當前速度與加速度，再外插未來軌跡。
        - ego_seq: (B,T_rf,4) 其中前兩維是 (ex,ey) = (dx,dy)/EGO_SCALE_M，對應最近歷史幀 -> 現在幀
        - 輸出 coarse_xy: (B,T,2)，座標系與你的 pred/gt 一致（第 0 維是側向、第 1 維是前向）。
        注意：coarse 仍只是 baseline，不追求取代影像分支；但它會比單步 CV 更平滑、更能反映近幾幀趨勢。
        """
        if T is None:
            T = self.n_future
        ...
        with autocast(enabled=False):
            ...
            # wrapper 中最後一幀是 current，因此歷史點落在負時間軸；取負號後可視為「過去軌跡」。
            past_xy = (-ego_seq[..., 0:2].float() * ego_scale_m).to(solve_dtype)  # (B,T_rf,2)
            hist_t = (torch.arange(T_rf, device=device, dtype=solve_dtype) - (T_rf - 1)) * dt  # [..., -dt, 0]
            future_t = torch.arange(1, T + 1, device=device, dtype=solve_dtype) * dt

            # 最近幾幀權重大一些，但仍使用整段歷史。
            hist_w = torch.linspace(0.7, 1.3, T_rf, device=device, dtype=solve_dtype)
            sqrt_w = hist_w.sqrt()

            if T_rf >= 3:
                # x(t) = v * t + 0.5 * a * t^2, 並強制 x(0)=0，避免 coarse 在當前點漂移。
                design_hist = torch.stack([hist_t, 0.5 * hist_t.pow(2)], dim=-1)   # (T_rf,2)
                design_fut = torch.stack([future_t, 0.5 * future_t.pow(2)], dim=-1)  # (T,2)
            else:
                design_hist = hist_t.unsqueeze(-1)    # 退化成 constant velocity
                design_fut = future_t.unsqueeze(-1)

            design_hist_w = design_hist * sqrt_w.unsqueeze(-1)
            past_xy_w = past_xy * sqrt_w.view(1, T_rf, 1)

            lhs = design_hist_w.transpose(0, 1) @ design_hist_w
            reg = 1e-4 * torch.eye(lhs.shape[0], device=device, dtype=solve_dtype)
            rhs = torch.einsum('tp,btc->bpc', design_hist_w, past_xy_w).to(solve_dtype)
            coeff = torch.linalg.solve(lhs.unsqueeze(0) + reg.unsqueeze(0), rhs)  # (B,P,2)

            coarse_xy = torch.einsum('tp,bpc->btc', design_fut, coeff)

            # 限制加速度外插不要過頭，與純速度模型做少量混合，穩住長期預測。
            if coeff.shape[1] == 2:
                vel_only = future_t.view(1, T, 1) * coeff[:, 0:1, :]
                blend = float(getattr(self, "COARSE_QUAD_BLEND", 0.7))
                coarse_xy = blend * coarse_xy + (1.0 - blend) * vel_only

        return coarse_xy.to(dtype)
    ...
    def generate_autoregressive(
        ...
        for t in range(T):
            ...
            ar_traj.append(ar_xy.unsqueeze(1))  # (B,1,2)
        ...
        if len(ar_traj) > 0:
            self.last_ar_xy = torch.cat(ar_traj, dim=1).detach()   # (B,T,2)
        else:
            self.last_ar_xy = None
                                         # (B,T,2)
...
class VLM_STP3_Gen(nn.Module):
    ...
    def make_soft_cost_map(
        self,
        occupancy: torch.Tensor,
        blur_iters: int = 4,          # ← 原本 2，建議先 4（甚至 6）
        inflation_cells: int = 2,     # ← 新增：先試 2（代表半徑 2 cells）
        clamp01: bool = True,
    ) -> torch.Tensor:
        """
        將 0/1 occupancy 轉 soft cost map
        - inflation_cells：先做 max-pool 膨脹（等價把障礙物變胖）
        - blur_iters：再用 avg_pool 做距離衰減（讓靠近也有 cost）
        """
        B, T, H, W = occupancy.shape
        # print("H, W",H, W)
        cost = occupancy.float().view(B * T, 1, H, W)

        # 1) inflation：讓障礙物先「變胖」
        if inflation_cells and inflation_cells > 0:
            k = 2 * inflation_cells + 1
            cost = F.max_pool2d(cost, kernel_size=k, stride=1, padding=inflation_cells)

        # 2) blur：近似距離衰減（多做幾次半徑就更大）
    ...
    def collision_loss_soft(self, trajs_xy: torch.Tensor, occupancy: torch.Tensor, return_per_t: bool = False) -> torch.Tensor:
        ...
        cost_map = self.make_soft_cost_map(
            occ_inf,
            blur_iters=int(getattr(self.cfg, "COST_BLUR_ITERS", 4)),
            inflation_cells=int(getattr(self.cfg, "COST_INFLATE_CELLS", 2)),
        )
    ...
    def box_collision_loss_soft(self, trajs_xy: torch.Tensor, occupancy: torch.Tensor, return_per_t: bool = False) -> torch.Tensor:
    ...
    def occupancy_box_collision_rate(self, trajs_xy: torch.Tensor, occupancy: torch.Tensor) -> torch.Tensor:
        risk_t = self.box_collision_loss_soft(trajs_xy, occupancy, return_per_t=True)
        return (risk_t > 0.5).float().mean()
    ...
    def planning(self, *, bev_rgbs, trajs, gt_trajs, commands, target_points, occupancy=None):
        ...
        # GT 每步位移統計（m/step）
        # =========================
        gt_xy = gt_trajs[..., :2]  # (B,T,2)
        gt_step = torch.norm(gt_xy[:, 1:] - gt_xy[:, :-1], dim=-1)  # (B,T-1)

        # 展平成一維，統計分位數
        gt_step_flat = gt_step.reshape(-1)

        if gt_step_flat.numel() > 0:
            gt_step_mean = gt_step_flat.mean()
            gt_step_p90  = torch.quantile(gt_step_flat, 0.90)
            gt_step_p95  = torch.quantile(gt_step_flat, 0.95)
            gt_step_max  = gt_step_flat.max()
        else:
            gt_step_mean = torch.tensor(0.0, device=device)
            gt_step_p90  = torch.tensor(0.0, device=device)
            gt_step_p95  = torch.tensor(0.0, device=device)
            gt_step_max  = torch.tensor(0.0, device=device)

        # ===== DEBUG：只印一次，避免刷屏 =====

        self._dbg_gt_step_printed = True
        # 取第 0 筆樣本看一下連續差
        s = gt_step[0].detach().cpu()  # (T-1,)
        # print("[GT step] first sample min/mean/max:", float(s.min()), float(s.mean()), float(s.max()))
        # print("[GT xy] first 3 points:", gt_xy[0, :3].detach().cpu().numpy())
        # print("[GT xy] last 3 points:",  gt_xy[0, -3:].detach().cpu().numpy())

        # print("gt_step_mean",gt_step_mean)
        ...
        box_coll_rate = torch.tensor(0.0, device=device)
        risk_t = None  # 你原本用來做 risk_mask / gate_risk 的，這裡保留「final 的 per-t risk」
        box_risk_t = None

        if occupancy is not None:
            risk_mean = torch.tensor(0.0, device=device)
            risk_max  = torch.tensor(0.0, device=device)

            # 0) 取 AR 軌跡（影像分支）
            ar_xy = getattr(self.vlm, "last_ar_xy", None)  # (B,T,2)
            if ar_xy is None:
                # 萬一某些情況沒存到，就退回用 pred
                ar_xy = pred[..., :2]

            # 1) 兩條風險序列：final 與 AR
            risk_final_t = self.collision_loss_soft(pred[..., :2], occupancy, return_per_t=True)  # (B,T)
            risk_ar_t    = self.collision_loss_soft(ar_xy,          occupancy, return_per_t=True)  # (B,T)
            box_final_t  = self.box_collision_loss_soft(pred[..., :2], occupancy, return_per_t=True)
            box_ar_t     = self.box_collision_loss_soft(ar_xy,          occupancy, return_per_t=True)

            # 2) 你後面還要用 risk_t 做 risk_mask / gate_risk / vis_sens，所以 risk_t 仍用 final 的
            risk_t = risk_final_t
            box_risk_t = box_final_t

            # 3) 聚合超參（建議從 cfg 讀，避免你以為改了其實沒生效）
            risk_pow  = float(getattr(self.cfg, "RISK_POW", 2.0))
            risk_mode = str(getattr(self.cfg, "RISK_REDUCE", "topk"))
            topk_frac = float(getattr(self.cfg, "RISK_TOPK_FRAC", 0.2))

            def reduce_risk(risk_per_t: torch.Tensor, *, risk_pow: float, risk_mode: str, topk_frac: float) -> torch.Tensor:
                r = risk_per_t.clamp(min=0.0) ** risk_pow
                if risk_mode == "mean":
                    return r.mean()
                elif risk_mode == "max":
                    return r.max(dim=1).values.mean()
                    ...
                    raise ValueError(f"Unknown risk reduce mode={risk_mode}")

            coll_final = reduce_risk(risk_final_t, risk_pow=risk_pow, risk_mode=risk_mode, topk_frac=topk_frac)
            coll_ar    = reduce_risk(risk_ar_t, risk_pow=risk_pow, risk_mode=risk_mode, topk_frac=topk_frac)

            box_risk_pow  = float(getattr(self.cfg, "BOX_RISK_POW", 2.0))
            box_risk_mode = str(getattr(self.cfg, "BOX_RISK_REDUCE", "topk"))
            box_topk_frac = float(getattr(self.cfg, "BOX_RISK_TOPK_FRAC", 0.34))
            box_coll_final = reduce_risk(box_final_t, risk_pow=box_risk_pow, risk_mode=box_risk_mode, topk_frac=box_topk_frac)
            box_coll_ar    = reduce_risk(box_ar_t, risk_pow=box_risk_pow, risk_mode=box_risk_mode, topk_frac=box_topk_frac)

            # 4) 合併：final + α * AR
            alpha_ar = float(getattr(self.cfg, "LOSS_COL_AR_ALPHA", 1.0))
            coll_rate = coll_final + alpha_ar * coll_ar

            alpha_box_ar = float(getattr(self.cfg, "LOSS_BOX_COL_AR_ALPHA", 1.0))
            box_coll_rate = box_coll_final + alpha_box_ar * box_coll_ar

            # 5) （可選）記錄一些 scalar（避免 tensorboard 爆，務必是純量）
            risk_mean = risk_t.mean()
            risk_max  = risk_t.max(dim=1).values.mean()
            box_risk_mean = box_risk_t.mean()
            box_risk_max = box_risk_t.max(dim=1).values.mean()

            if self.training and (not hasattr(self, "_dbg_col_once")):
                self._dbg_col_once = True
                print(f"[DBG col] coll_final={float(coll_final):.4f}  coll_ar={float(coll_ar):.4f}  "
                    f"alpha={alpha_ar:.2f}  coll_rate={float(coll_rate):.4f}")
                print(f"[DBG box] box_final={float(box_coll_final):.4f}  box_ar={float(box_coll_ar):.4f}  "
                    f"alpha={alpha_box_ar:.2f}  box_coll_rate={float(box_coll_rate):.4f}")
        ...
        if cmd_forward.any():
            ...
            if occupancy is not None and risk_t is not None:
                # ★ 修改變數名稱為 fwd_risk_max
                fwd_risk_max = risk_t[cmd_forward].max(dim=-1)[0].detach()
                # 風險越高，容忍的 margin 越大
                dynamic_margin = margin_fwd + 2.0 * fwd_risk_max 
        ...
        if turn_mask.any():
            ...
            dir_acc_turn = torch.tensor(0.0, device=device)
        ...
        if (is_inference) or (occupancy is None):
            ...
            hard_box_col = torch.tensor(0.0, device=device)
        else:
            with torch.no_grad():
                hard_col = self.occupancy_collision_rate(pred[..., :2], occupancy)
                hard_box_col = self.occupancy_box_collision_rate(pred[..., :2], occupancy)

        simple_loss_mode = bool(getattr(self.cfg, 'SIMPLE_LOSS_MODE', True))

        lam_l2 = float(getattr(self.cfg, 'LOSS_L2_W', 8.0))
        lam_col = float(getattr(self.cfg, 'LOSS_COL_W', 30.0))
        lam_box_col = float(getattr(self.cfg, 'LOSS_BOX_COL_W', 10.0))
        ...
        if simple_loss_mode:
            # 用少數幾個核心 loss，避免輔助項互相拉扯。
            lam_l2 = float(getattr(self.cfg, 'SIMPLE_LOSS_L2_W', lam_l2))
            lam_col = float(getattr(self.cfg, 'SIMPLE_LOSS_COL_W', lam_col))
            lam_box_col = float(getattr(self.cfg, 'SIMPLE_LOSS_BOX_COL_W', lam_box_col))
            lam_dir = float(getattr(self.cfg, 'SIMPLE_LOSS_DIR_W', 2.0))
            lam_smo = float(getattr(self.cfg, 'SIMPLE_LOSS_SMO_W', 0.05))
            lam_vel = 0.0
            lam_coarse = 0.0
        ...
        if (risk_t is not None):
            ...
            if (not simple_loss_mode) and self.training and lam_vis > 0 and (self._train_step % max(1, vis_every) == 0) and (risk_mask.sum() > 0):
                ...
                with torch.no_grad():
                    ...
                    pred_blk = self.vlm.generate_autoregressive(
                        ...
                        bx=self.bx, dx=self.dx, bev_dim=self.bev_dim
                    ).to(device)
        ...
        if (not simple_loss_mode) and (mix_gate is not None) and (risk_mask is not None) and (lam_gate_risk > 0) and (risk_mask.sum() > 0):
            gate_risk_loss = (((1.0 - mix_gate) * risk_mask)).sum() / (risk_mask.sum() + 1e-6)
            loss = loss + lam_gate_risk * gate_risk_loss

        if (not simple_loss_mode) and self.training and (mix_gate is not None) and (keep_t is not None) and (lam_gate_drop > 0):
            # print("455456546")
        ...
        ar_xy = getattr(self.vlm, "last_ar_xy", None)         # (B,T,2)
        coarse_xy = getattr(self.vlm, "last_coarse_xy", None) # (B,T,2)

        ar_coarse_mean = torch.tensor(0.0, device=device)
        ar_coarse_mean_on_risk = torch.tensor(0.0, device=device)

        if (ar_xy is not None) and (coarse_xy is not None):
            diff = torch.norm(ar_xy - coarse_xy, dim=-1)  # (B,T)
            ar_coarse_mean = diff.mean()

            if risk_t is not None:
                # 跟你 vis_sens 用同一個 thres，避免指標定義分裂
                risk_thres = float(getattr(self.cfg, "VIS_RISK_THRES", 0.15))
                mask = (risk_t > risk_thres).float()  # (B,T)
                ar_coarse_mean_on_risk = (diff * mask).sum() / (mask.sum() + 1e-6)

        # =========================
        # AR vs Coarse：左右/前後分量統計（寫入 TensorBoard）
        # =========================
        abs_dx_mean = torch.tensor(0.0, device=device)
        abs_dy_mean = torch.tensor(0.0, device=device)
        abs_dx_mean_on_risk = torch.tensor(0.0, device=device)
        abs_dy_mean_on_risk = torch.tensor(0.0, device=device)
        dx_p95_on_risk = torch.tensor(0.0, device=device)

        if (ar_xy is not None) and (coarse_xy is not None):
            dxy = ar_xy - coarse_xy                 # (B,T,2)
            dx = dxy[..., 0].abs()                  # (B,T) lateral
            dy = dxy[..., 1].abs()                  # (B,T) longitudinal
            ...
            if risk_t is not None:
                ...
                abs_dx_mean_on_risk = (dx * mask).sum() / (mask.sum() + 1e-6)
                abs_dy_mean_on_risk = (dy * mask).sum() / (mask.sum() + 1e-6)

                # 95 分位（只看 risk 區段的 lateral 偏移，較貼近「最多偏多少」）
                dx_masked = dx[mask.bool()]
                if dx_masked.numel() > 0:
                    dx_p95_on_risk = torch.quantile(dx_masked, 0.95)
        ...
        #     self._dbg_i += 1
        #     gt_xy = gt_trajs[..., :2]
        #     gt_step = torch.norm(gt_xy[:, 1:] - gt_xy[:, :-1], dim=-1)
        #     print(f"[DBG] gt_xy_maxabs={float(gt_xy.abs().max()):.6f}, gt_step_mean={float(gt_step.mean()):.6f}")
        ...
        loss_dict = {
            ...
            "box_collision": box_coll_rate.detach(),
            "box_col_with_lam": (lam_box_col * box_coll_rate).detach(),
            "hard_box_collision": hard_box_col,
            "box_risk_mean": box_risk_mean.detach() if box_risk_t is not None else torch.tensor(0.0, device=device),
            "box_risk_max":  box_risk_max.detach() if box_risk_t is not None else torch.tensor(0.0, device=device),
            ...
            "risk_t_mean": risk_mean.detach() if risk_t is not None else torch.tensor(0.0, device=device),
            "risk_t_max":  risk_max.detach()  if risk_t is not None else torch.tensor(0.0, device=device),
            "ar_minus_coarse_mean": ar_coarse_mean.detach(),
            "ar_minus_coarse_mean_on_risk": ar_coarse_mean_on_risk.detach(),
            "gt_step_mean": gt_step_mean.detach(),
            "gt_step_p90":  gt_step_p90.detach(),
            "gt_step_p95":  gt_step_p95.detach(),
            "gt_step_max":  gt_step_max.detach(),
            "abs_dx_mean": abs_dx_mean.detach(),
            "abs_dy_mean": abs_dy_mean.detach(),
            "abs_dx_mean_on_risk": abs_dx_mean_on_risk.detach(),
            "abs_dy_mean_on_risk": abs_dy_mean_on_risk.detach(),
            "dx_p95_on_risk": dx_p95_on_risk.detach(),
        }

        # print("gt_step_mean.detach()",gt_step_mean.detach())
        ...
        # 限制 longitudinal 修正，不讓模型只靠改速度/前後來避障
        loss_dy_cap = torch.tensor(0.0, device=device)
        dy_exceed_ratio_on_risk = torch.tensor(0.0, device=device)
        risk_mask_ratio = torch.tensor(0.0, device=device)
        if (ar_xy is not None) and (coarse_xy is not None):
            dy = (ar_xy - coarse_xy)[..., 1]          # (B,T)
            dy_abs = dy.abs()

            # 只在 risk 區段限制（避免把正常跟車/加減速也打死）
            risk_thres = float(getattr(self.cfg, "VIS_RISK_THRES", 0.15))
            mask = (risk_t > risk_thres).float() if (risk_t is not None) else torch.ones_like(dy_abs)

            dy_cap = float(getattr(self.cfg, "DY_CAP_M", 1.0))   # 先設 1.0m
            loss_dy_cap = (torch.relu(dy_abs - dy_cap) * mask).sum() / (mask.sum() + 1e-6)

            if not simple_loss_mode:
                lam_dy_cap = float(getattr(self.cfg, "LOSS_DY_CAP_W", 5.0))  # 先 2.0，再看
                loss = loss + lam_dy_cap * loss_dy_cap
            loss_dict["loss_dy_cap"] = loss_dy_cap.detach()

            mask = (risk_t > risk_thres) if (risk_t is not None) else torch.ones_like(dy_abs, dtype=torch.bool)

            # 避免全 0 mask
            den = mask.float().sum() + 1e-6
            dy_exceed = (dy_abs > dy_cap) & mask
            dy_exceed_ratio_on_risk = dy_exceed.float().sum() / den
            risk_mask_ratio = mask.float().mean().detach()

        loss_dict["loss_dy_cap"] = loss_dy_cap.detach()
        loss_dict["dy_exceed_ratio_on_risk"] = dy_exceed_ratio_on_risk.detach()
        loss_dict["risk_mask_ratio"] = risk_mask_ratio

        gt_xy = gt_trajs[..., :2]              # (B,T,2)
        gt_x = gt_xy[..., 0].abs()             # (B,T)
        gt_dx = (gt_xy[:, 1:, 0] - gt_xy[:, :-1, 0]).abs()  # (B,T-1)

        loss_dict["gt_abs_x_mean"] = gt_x.mean().detach()
        loss_dict["gt_abs_x_p95"]  = torch.quantile(gt_x.reshape(-1), 0.95).detach() if gt_x.numel() > 0 else torch.tensor(0.0, device=device)
        loss_dict["gt_step_dx_mean"] = gt_dx.mean().detach() if gt_dx.numel() > 0 else torch.tensor(0.0, device=device)
        loss_dict["gt_step_dx_p95"]  = torch.quantile(gt_dx.reshape(-1), 0.95).detach() if gt_dx.numel() > 0 else torch.tensor(0.0, device=device)
        ...
        loss_dict["simple_loss_mode"] = torch.tensor(float(simple_loss_mode), device=device)
