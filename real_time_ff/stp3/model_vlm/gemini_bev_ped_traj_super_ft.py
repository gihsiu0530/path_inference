...
from stp3.model_vlm.gemini_bev import (
...
class VLM_STP3_Gen(BaseVLM_STP3_Gen):
    def __init__(self, cfg):
        ...
        self.vlm.ped_gate_min = float(getattr(cfg, 'PED_GATE_MIN', self.vlm.ped_gate_min))
        self.vlm.ped_gate_init_bias = float(getattr(cfg, 'PED_GATE_INIT_BIAS', self.vlm.ped_gate_init_bias))
        self.vlm.ped_start_step = int(getattr(cfg, 'PED_START_STEP', self.vlm.ped_start_step))
        self.vlm.ped_ramp_steepness = float(getattr(cfg, 'PED_RAMP_STEEPNESS', self.vlm.ped_ramp_steepness))
        self.vlm.ped_bev_gate_min = float(getattr(cfg, 'PED_BEV_GATE_MIN', self.vlm.ped_bev_gate_min))
        nn.init.constant_(self.vlm.ped_fuse_gate[2].bias, self.vlm.ped_gate_init_bias)
        self.vlm.apply_visual_unfreeze(self.vlm.VIS_UNFREEZE_K)

        print('model : gemini_bev_ped_traj_super_ft')
    ...
    def _planning_base_super_ft(self, *, bev_rgbs, trajs, gt_trajs, commands, target_points, occupancy=None):
        ...
        with torch.no_grad():
            ...
            scenario_tag = torch.tensor(0.0, device=device)

            if is_inference:
                curr_img_raw = self._last_rgb_seq[0, -1, ...]
                curr_img_input = curr_img_raw.unsqueeze(0) if curr_img_raw.dim() == 3 else curr_img_raw
                ...
                vlm_target_speed = None
                vlm_amplify_scale = 1.0

                if scenario == "BUMP":
                    final_commands = ["SLOW"] * len(commands)
                    vlm_target_speed = 1.5
                elif scenario in ["OBSTACLE", "OBSTACLE_RIGHT", "OBSTACLE_LEFT"]:
                    final_commands = ["SLOW"] * len(commands)
                    vlm_amplify_scale = 2.0
                ...
                if vlm_target_speed is not None or vlm_amplify_scale > 1.0:
        ...
        if occupancy is not None:
            risk_t = self.collision_loss_soft(pred[..., :2], occupancy, return_per_t=True)
        ...
        margin_turn = getattr(self.cfg, 'DIR_MARGIN_TURN', 1.8)
        margin_fwd = getattr(self.cfg, 'DIR_MARGIN_FORWARD', 2.0)

        dir_loss = torch.tensor(0.0, device=device)
        if cmd_right.any():
            x_r = pred_last_x[cmd_right]
            dir_loss = dir_loss + F.relu(margin_turn - x_r).mean()
        if cmd_left.any():
            x_l = pred_last_x[cmd_left]
            dir_loss = dir_loss + F.relu(x_l + margin_turn).mean()
        if cmd_forward.any():
        ...
        lam_col = getattr(self.cfg, 'LOSS_COL_W', 30.0)
        lam_smo = getattr(self.cfg, 'LOSS_SMO_W', 0.1)
        lam_vel = getattr(self.cfg, 'LOSS_VEL_W', 0.6)
        lam_dir = getattr(self.cfg, 'LOSS_DIR_W', 8.0)
        lam_coarse = getattr(self.cfg, 'LOSS_COARSE_W', 0.0)
        ...
        vis_every = int(getattr(self.cfg, "VIS_SENS_EVERY", 4))
        risk_thres = float(getattr(self.cfg, "VIS_RISK_THRES", 0.15))
        vis_margin = float(getattr(self.cfg, "VIS_SENS_MARGIN", 0.6))

        vis_sens = torch.tensor(0.0, device=device)
        risk_mask = None
        if risk_t is not None:
            risk_w = torch.clamp(risk_t.detach(), 0.0, 1.0)
            ...
            if self.training and lam_vis > 0 and (self._train_step % max(1, vis_every) == 0) and (risk_mask.sum() > 0):
                ...
                diff = torch.norm(pred[..., :2] - pred_blk[..., :2], dim=-1)
        ...
        keep_t = getattr(self.vlm, "last_coarse_keep_t", None)
        ...
        if self.training and (mix_gate is not None) and (keep_t is not None) and (lam_gate_drop > 0):
            drop = (keep_t.squeeze(-1) < 0.5).float()
            if drop.sum() > 0:
        ...
        return loss, pred, final_traj, scenario_tag, loss_dict
    ...
    def planning(self, *, bev_rgbs, trajs, gt_trajs, commands, target_points, occupancy=None, ped_bev_points=None, ped_bev_valid_steps=None):
        ...
        loss, pred, final_traj, resample_traj, loss_dict = self._planning_base_super_ft(
            bev_rgbs=bev_rgbs,
            trajs=trajs,
            gt_trajs=gt_trajs,
            commands=commands,
