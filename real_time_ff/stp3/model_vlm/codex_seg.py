...
class VLM_STP3_Gen(nn.Module):
    ...
    def offroad_loss_soft(self, trajs_xy: torch.Tensor, drivable_mask: torch.Tensor, return_per_t: bool = False) -> torch.Tensor:
        """
        Penalize predicted trajectory points that leave the BEV drivable area.
        drivable_mask: (B,H,W) or (B,T,H,W), where 1 means drivable.
        """
        B, T, _ = trajs_xy.shape
        if drivable_mask.dim() == 3:
            drivable_mask = drivable_mask[:, None].expand(-1, T, -1, -1)
        elif drivable_mask.dim() == 4 and drivable_mask.shape[1] != T:
            drivable_mask = drivable_mask[:, :1].expand(-1, T, -1, -1)
        drivable_mask = drivable_mask.to(device=trajs_xy.device, dtype=torch.float32).contiguous()

        H, W = drivable_mask.shape[-2:]
        drivable = drivable_mask.view(B * T, 1, H, W)

        # Allow small map/annotation mismatch around lane boundaries before penalizing.
        dilate_cells = int(getattr(self.cfg, 'OFFROAD_DRIVABLE_DILATE_CELLS', 0))
        if dilate_cells > 0:
            kernel = 2 * dilate_cells + 1
            drivable = F.max_pool2d(drivable, kernel_size=kernel, stride=1, padding=dilate_cells)

        offroad_cost = 1.0 - drivable
        blur_iters = int(getattr(self.cfg, 'OFFROAD_BLUR_ITERS', 0))
        for _ in range(max(0, blur_iters)):
            offroad_cost = F.avg_pool2d(offroad_cost, kernel_size=3, stride=1, padding=1)
        offroad_cost = torch.clamp(offroad_cost, 0.0, 1.0)

        yy = (trajs_xy[..., 1] - self.bx[0]) / self.dx[0]
        xx = (trajs_xy[..., 0] - self.bx[1]) / self.dx[1]
        x_norm = (xx / (W - 1)) * 2 - 1
        y_norm = (yy / (H - 1)) * 2 - 1
        grid = torch.stack([x_norm, y_norm], dim=-1).view(B * T, 1, 1, 2)
    ...
    def planning(self, *, bev_rgbs, trajs, gt_trajs, commands, target_points, occupancy=None, drivable_mask=None):
        ...
        offroad = torch.tensor(0.0, device=device)
        offroad_t = None
        offroad_mean = torch.tensor(0.0, device=device)
        offroad_max = torch.tensor(0.0, device=device)
        if drivable_mask is not None:
            offroad_t = self.offroad_loss_soft(pred[..., :2], drivable_mask, return_per_t=True)
            offroad_power = float(getattr(self.cfg, 'OFFROAD_POWER', 1.0))
            offroad_penalty = offroad_t ** offroad_power
            offroad_mean = offroad_penalty.mean()
            offroad_max = offroad_penalty.max(dim=1)[0].mean()
            offroad_max_w = float(getattr(self.cfg, 'OFFROAD_MAX_W', 0.5))
            offroad = (1.0 - offroad_max_w) * offroad_mean + offroad_max_w * offroad_max
        ...
        lam_offroad = getattr(self.cfg, 'LOSS_OFFROAD_W', 10.0)
        ...
        loss_dict = {
            ...
            "offroad_with_lam": lam_offroad * offroad,
