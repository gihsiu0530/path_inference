...
class VLM_Generative(nn.Module):
    ...
    def __init__(self, clip_name: str, device: torch.device, n_future: int, input_size: int = 224,
        ...
        self.seg_cnn_encoder = nn.Sequential(
            ...
            nn.BatchNorm2d(128),
            nn.GELU(),
            nn.Conv2d(128, self.vis_width, kernel_size=1),
            nn.GELU(),
        )
        self.seg_id_embedding = nn.Embedding(
            int(getattr(self, "SEG_NUM_CLASSES", 4)),
            int(getattr(self, "SEG_EMBED_DIM", 32)),
        )
        self.seg_id_cnn_encoder = nn.Sequential(
            nn.Conv2d(int(getattr(self, "SEG_EMBED_DIM", 32)), 64, kernel_size=5, stride=2, padding=2),
            nn.BatchNorm2d(64),
            nn.GELU(),
            nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(128),
        ...
        self.seg_cnn_ln = nn.LayerNorm(self.vis_width)
        self.depth_cnn_encoder = nn.Sequential(
        ...
        self.depth_cnn_ln = nn.LayerNorm(self.vis_width)

        # ★ 新增：BEV 解碼器 (將 2D 的 CLIP 特徵轉成鳥瞰圖)
        # 預期輸入：(B, vis_width*4, H_f, W_f) -> 輸出：(B, vis_width, H_bev, W_bev)
        self.bev_decoder = nn.Sequential(
            nn.Conv2d(self.vis_width * 4, self.vis_width, kernel_size=3, padding=1),
    ...
    def generate_autoregressive(
        ...
        # ★ 修改：接收兩個回傳值
        # ★ 修改：接收兩個回傳值
        vis_tokens, bev_input = self.build_vis_tokens(
            rgb_seq, seg_seq, ego_seq, seg_id_seq=seg_id_seq, depth_seq=depth_seq
        )
    ...
    def _preprocess_seg_cnn_tensor(self, imgs: torch.Tensor) -> torch.Tensor:
        if imgs.dim() == 4 and imgs.shape[-1] == 3:
            imgs = imgs.permute(0, 3, 1, 2)
        x = imgs.contiguous().to(torch.float32) / 255.0
        if x.shape[-1] != self.input_size or x.shape[-2] != self.input_size:
            x = F.interpolate(x, size=(self.input_size, self.input_size), mode="nearest")
        return x

    def _encode_seg_id_tensor(self, seg_id: torch.Tensor, target_hw: Tuple[int, int]) -> torch.Tensor:
        if seg_id.dim() == 4 and seg_id.shape[1] == 1:
            seg_id = seg_id[:, 0]
        seg_id = seg_id.to(self.device, non_blocking=True).long()
        emb = self.seg_id_embedding(seg_id).permute(0, 3, 1, 2).contiguous()
        if emb.shape[-2:] != target_hw:
            emb = F.interpolate(emb, size=target_hw, mode="nearest")
        return self.seg_id_cnn_encoder(emb)

    def _encode_depth_tensor(self, depth: torch.Tensor, target_hw: Tuple[int, int]) -> torch.Tensor:
        if depth.dim() == 3:
            depth = depth.unsqueeze(1)
        elif depth.dim() == 4 and depth.shape[-1] != target_hw[-1] and depth.shape[1] != 1:
            depth = depth.unsqueeze(1)
        depth = depth.to(self.device, non_blocking=True).float()
        depth = torch.nan_to_num(depth, nan=0.0, posinf=80.0, neginf=0.0)
        depth = torch.clamp(depth, min=0.0, max=80.0) / 80.0
        if depth.shape[-2:] != target_hw:
            depth = F.interpolate(depth, size=target_hw, mode="bilinear", align_corners=False)
        return self.depth_cnn_encoder(depth)
    ...
    def build_vis_tokens(self, rgb_seq, seg_seq, ego_seq: torch.Tensor, seg_id_seq: torch.Tensor = None, depth_seq: torch.Tensor = None) -> torch.Tensor:
        """
        將 (B,T_rf,...) 的 RGB/SEG 與 ego_seq 轉成單一 token set: (B, S_all, C)
        v1：使用向量化版本（一次 encode_image），速度更快、也更穩定。
        """
        if seg_id_seq is None:
            raise ValueError("codex_change requires seg_id_seq / seg_id_224_seq, but got None.")
        if depth_seq is None:
            raise ValueError("codex_change requires depth_seq / depth_224_seq, but got None.")
        ...
        if seg_id_seq is not None and not torch.is_tensor(seg_id_seq):
            seg_id_seq = torch.from_numpy(seg_id_seq)
        if not torch.is_tensor(depth_seq):
            depth_seq = torch.from_numpy(depth_seq)
        ...
        seg_cnn_bt = self._preprocess_seg_cnn_tensor(seg_bt)
        ...
        seg_cnn_feat = self._encode_seg_id_tensor(seg_id_bt, target_hw=(self.input_size, self.input_size))
        depth_cnn_feat = self._encode_depth_tensor(depth_bt, target_hw=(self.input_size, self.input_size))
        seg_cnn_feat = F.adaptive_avg_pool2d(seg_cnn_feat, output_size=(H_f, W_f))
        depth_cnn_feat = F.adaptive_avg_pool2d(depth_cnn_feat, output_size=(H_f, W_f))
        seg_cnn_feat = seg_cnn_feat.view(B, T_rf, self.vis_width, H_f, W_f)
        depth_cnn_feat = depth_cnn_feat.view(B, T_rf, self.vis_width, H_f, W_f)
        seg_cnn_tok = seg_cnn_feat.permute(0, 1, 3, 4, 2).reshape(B, T_rf, H_f * W_f, self.vis_width)
        depth_cnn_tok = depth_cnn_feat.permute(0, 1, 3, 4, 2).reshape(B, T_rf, H_f * W_f, self.vis_width)
        seg_cnn_tok = self.seg_cnn_ln(seg_cnn_tok)
        depth_cnn_tok = self.depth_cnn_ln(depth_cnn_tok)
        ...
        tok_1d = torch.cat([tok_rgb_1d, tok_seg_1d, seg_cnn_tok.to(tok_all.dtype), depth_cnn_tok.to(tok_all.dtype)], dim=2)
        
        ego_embed = self.ego_mlp(ego_seq.to(device))   # (B, T_rf, C)
        tok_1d = tok_1d + ego_embed.unsqueeze(2)
        tok_1d_flat = tok_1d.reshape(B, -1, self.vis_width)
        ...
        seg_cnn_spatial = seg_cnn_feat[:, -1].permute(0, 2, 3, 1).to(tok_all.dtype)
        depth_cnn_spatial = depth_cnn_feat[:, -1].permute(0, 2, 3, 1).to(tok_all.dtype)
        
        # Concat 並 Permute 成 CNN 預期的 (B, C, H, W)
        bev_input = torch.cat([rgb_spatial, seg_spatial, seg_cnn_spatial, depth_cnn_spatial], dim=-1)
        bev_input = bev_input.permute(0, 3, 1, 2).contiguous()
    ...
    def generate(self, rgb_seq, seg_seq, ego_seq, commands, gt_trajs: torch.Tensor = None, bx=None, dx=None, bev_dim=None, seg_id_seq=None, depth_seq=None):
...
class VLM_STP3_Gen(nn.Module):
    ...
    def forward(self, image, intrinsics, extrinsics, future_egomotion, *, rgb_224_seq, seg_224_seq, seg_id_224_seq=None, depth_224_seq=None):
        if seg_id_224_seq is None:
            raise ValueError(
                "codex_change requires batch['seg_id_224_seq']. "
                "Use NuscenesData_change or provide *_cls4_224.npy class-id maps."
            )
        if depth_224_seq is None:
            raise ValueError(
                "codex_change requires batch['depth_224_seq']. "
                "Use NuscenesData_change and provide precomputed depth npy maps."
            )
    ...
    def image_obstacle_cost_map(self, seg_id_seq: torch.Tensor, depth_seq: torch.Tensor) -> torch.Tensor:
        seg = seg_id_seq[:, -1].long()
        depth = depth_seq[:, -1].float()
        depth = torch.nan_to_num(depth, nan=0.0, posinf=0.0, neginf=0.0)

        obstacle_classes = getattr(self.cfg, "IMG_OBS_CLASSES", [1, 2, 3])
        if isinstance(obstacle_classes, str):
            obstacle_classes = [int(x) for x in obstacle_classes.split(",") if x.strip()]
        obstacle = torch.zeros_like(seg, dtype=torch.bool)
        for cls_id in obstacle_classes:
            obstacle = obstacle | (seg == int(cls_id))
        obstacle = obstacle.float().unsqueeze(1)

        dilate = int(getattr(self.cfg, "IMG_OBS_DILATE", 9))
        if dilate > 1 and dilate % 2 == 0:
            dilate += 1
        if dilate > 1:
            pad = dilate // 2
            obstacle = F.max_pool2d(obstacle, kernel_size=dilate, stride=1, padding=pad)

        B, _, H, W = obstacle.shape
        closeness = torch.zeros(B, H, W, device=depth.device, dtype=depth.dtype)
        low_pct = float(getattr(self.cfg, "IMG_OBS_DEPTH_LOW_PCT", 0.02))
        high_pct = float(getattr(self.cfg, "IMG_OBS_DEPTH_HIGH_PCT", 0.98))
        for b in range(B):
            valid = depth[b] > 0
            if not valid.any():
                continue
            vals = depth[b][valid]
            lo = torch.quantile(vals.float(), low_pct)
            hi = torch.quantile(vals.float(), high_pct)
            close = (depth[b] - lo) / torch.clamp(hi - lo, min=1e-6)
            closeness[b] = close.clamp(0.0, 1.0)

        gamma = float(getattr(self.cfg, "IMG_OBS_DEPTH_GAMMA", 1.0))
        closeness = closeness.clamp(0.0, 1.0).pow(gamma).unsqueeze(1)

        yy = torch.linspace(0.0, 1.0, H, device=depth.device, dtype=depth.dtype).view(1, 1, H, 1)
        xx = torch.linspace(0.0, 1.0, W, device=depth.device, dtype=depth.dtype).view(1, 1, 1, W)
        center_width = float(getattr(self.cfg, "IMG_OBS_CENTER_WIDTH", 1.0))
        center_power = float(getattr(self.cfg, "IMG_OBS_CENTER_POWER", 0.3))
        lower_power = float(getattr(self.cfg, "IMG_OBS_LOWER_POWER", 1.0))
        x_norm = (xx - 0.5).abs() / max(center_width / 2.0, 1e-6)
        center_weight = (1.0 - x_norm).clamp(0.0, 1.0).pow(center_power)
        lower_weight = yy.clamp(0.0, 1.0).pow(lower_power)
        pos_weight = lower_weight * (0.25 + 0.75 * center_weight)

        risk = obstacle * closeness * pos_weight
        blur = int(getattr(self.cfg, "IMG_OBS_BLUR", 7))
        if blur > 1 and blur % 2 == 0:
            blur += 1
        if blur > 1:
            pad = blur // 2
            risk = F.avg_pool2d(risk, kernel_size=blur, stride=1, padding=pad)
        return risk.clamp(0.0, 1.0)

    def image_obstacle_traj_loss(self, trajs_xy: torch.Tensor, cost_map: torch.Tensor):
        B, T, _ = trajs_xy.shape
        _, _, H, W = cost_map.shape
        y_min = float(getattr(self.cfg, "IMG_OBS_Y_MIN", 0.5))
        y_max = float(getattr(self.cfg, "IMG_OBS_Y_MAX", 8.0))
        u_center = float(getattr(self.cfg, "IMG_OBS_U_CENTER", 112.0))
        u_scale = float(getattr(self.cfg, "IMG_OBS_U_SCALE", 26.0))
        v_bottom = float(getattr(self.cfg, "IMG_OBS_V_BOTTOM", 218.0))
        v_top = float(getattr(self.cfg, "IMG_OBS_V_TOP", 92.0))

        x = trajs_xy[..., 0]
        y = trajs_xy[..., 1]
        progress = ((y - y_min) / max(y_max - y_min, 1e-6)).clamp(0.0, 1.0)
        u = u_center + x * u_scale
        v = v_bottom - (v_bottom - v_top) * torch.sqrt(progress.clamp_min(0.0))

        grid_x = (u / max(W - 1, 1)) * 2.0 - 1.0
        grid_y = (v / max(H - 1, 1)) * 2.0 - 1.0
        grid = torch.stack([grid_x, grid_y], dim=-1).view(B, T, 1, 2)
        sampled = F.grid_sample(cost_map, grid, mode="bilinear", padding_mode="zeros", align_corners=True)
        sampled = sampled[:, 0, :, 0]
        valid = ((y >= y_min) & (y <= y_max)).to(sampled.dtype)
        loss = (sampled * valid).sum() / (valid.sum() + 1e-6)
        return loss, sampled
    ...
    def planning(self, *, bev_rgbs, trajs, gt_trajs, commands, target_points, occupancy=None, drivable_mask=None):
        ...
        scenario = "TRAIN" if self.training else "UNKNOWN"
        ...
        pred = self.vlm.generate(
            ...
            seg_id_seq=getattr(self, "_last_seg_id_seq", None),
            depth_seq=getattr(self, "_last_depth_seq", None),
        ).to(device)
        ...
        with torch.no_grad():
            ...
            if is_inference:
                ...
                vlm_target_speed = None 
                vlm_amplify_scale = 1.0  # ▼ 新增：預設橫向位移不放大 (1.0倍)
        ...
        img_obs_loss = torch.tensor(0.0, device=device)
        img_obs_sample_mean = torch.tensor(0.0, device=device)
        img_obs_map_max = torch.tensor(0.0, device=device)
        lam_img_obs = float(getattr(self.cfg, "LOSS_IMG_OBS_W", 3.0))
        if lam_img_obs > 0 and getattr(self, "_last_seg_id_seq", None) is not None and getattr(self, "_last_depth_seq", None) is not None:
            img_obs_map = self.image_obstacle_cost_map(self._last_seg_id_seq.to(device), self._last_depth_seq.to(device))
            img_obs_loss, img_obs_samples = self.image_obstacle_traj_loss(pred[..., :2], img_obs_map)
            img_obs_clamp = float(getattr(self.cfg, "IMG_OBS_LOSS_CLAMP", 0.2))
            if img_obs_clamp > 0:
                img_obs_loss = img_obs_loss.clamp(max=img_obs_clamp)
            img_obs_sample_mean = img_obs_samples.mean().detach()
            img_obs_map_max = img_obs_map.amax(dim=(-1, -2, -3)).mean().detach()
            loss = loss + lam_img_obs * img_obs_loss
        ...
        lam_vis = float(getattr(self.cfg, "LOSS_VIS_SENS_W", 0.0))
        ...
        seg_sens = torch.tensor(0.0, device=device)
        risk_mask = None
        if (risk_t is not None):
            ...
            if self.training and lam_vis > 0 and (self._train_step % max(1, vis_every) == 0) and (risk_mask.sum() > 0):
                ...
                depth_blk = torch.zeros_like(self._last_depth_seq)

                with torch.no_grad():
                    pred_blk = self.vlm.generate_autoregressive(
                        rgb_blk, seg_blk, self._last_ego_seq.to(device), commands,
                        seg_id_seq=torch.zeros_like(self._last_seg_id_seq) if getattr(self, "_last_seg_id_seq", None) is not None else None,
                        depth_seq=depth_blk,
                        gt_trajs=None,                    # 不給 GT
            ...
            lam_seg = float(getattr(self.cfg, "LOSS_SEG_SENS_W", 0.0))
            seg_every = int(getattr(self.cfg, "SEG_SENS_EVERY", vis_every))
            seg_margin = float(getattr(self.cfg, "SEG_SENS_MARGIN", vis_margin))
            if self.training and lam_seg > 0 and (self._train_step % max(1, seg_every) == 0) and (risk_mask.sum() > 0):
                saved_mix = getattr(self.vlm, "last_mix_gate", None)
                saved_keep = getattr(self.vlm, "last_coarse_keep_t", None)

                seg_blk = torch.zeros_like(self._last_seg_seq)
                with torch.no_grad():
                    pred_no_seg = self.vlm.generate_autoregressive(
                        self._last_rgb_seq, seg_blk, self._last_ego_seq.to(device), commands,
                        seg_id_seq=torch.zeros_like(self._last_seg_id_seq) if getattr(self, "_last_seg_id_seq", None) is not None else None,
                        depth_seq=self._last_depth_seq,
                        gt_trajs=None,
                ...
                seg_diff = torch.norm(pred[..., :2] - pred_no_seg[..., :2], dim=-1)
                seg_sens = (F.relu(seg_margin - seg_diff) * risk_mask * risk_w).sum() / (risk_mask.sum() + 1e-6)
                loss = loss + lam_seg * seg_sens
        ...
        loss_dict = {
            ...
            "seg_sens": seg_sens.detach() if torch.is_tensor(seg_sens) else torch.tensor(0.0, device=device),
            "img_obs": img_obs_loss.detach(),
            "img_obs_with_lam": (lam_img_obs * img_obs_loss).detach(),
            "img_obs_sample_mean": img_obs_sample_mean,
            "img_obs_map_max": img_obs_map_max,
