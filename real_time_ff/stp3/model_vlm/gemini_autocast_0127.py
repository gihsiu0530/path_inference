    ...
    def generate_autoregressive(
        ...
        *,
        ...
        for t in range(T):
            ...
            # 取 gate 的「每個 sample 的平均 gate 值」： (B,)
            g_mean_per_sample = g.mean(dim=-1)

            # 也可選：更保守看分位數/最大值（看你需求）
            g_max_per_sample  = g.amax(dim=-1)        # (B,)
            gate_stats.append(torch.stack([g_mean_per_sample, g_max_per_sample], dim=-1))  # (B,2)
...
class VLM_STP3_Gen(nn.Module):
    ...
    def planning(self, *, bev_rgbs, trajs, gt_trajs, commands, target_points, occupancy=None):
        ...
        lam_coarse = getattr(self.cfg, 'LOSS_COARSE_W', 0.2)  # 粗軌跡 head 的 L2
        ...
        gate = getattr(self.vlm, "last_gate_stats", None)
        if gate is not None:
            # gate: (B,T,2)
            loss_dict["gate_mean_over_time"] = gate[..., 0].mean()      # scalar
            loss_dict["gate_mean_last5"] = gate[:, -5:, 0].mean() if gate.size(1) >= 5 else gate[..., 0].mean()
