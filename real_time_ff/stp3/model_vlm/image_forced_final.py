...
class VLM_STP3_Gen(BaseVLM_STP3_Gen):
    ...
    def planning(self, *, bev_rgbs, trajs, gt_trajs, commands, target_points, occupancy=None, ped_bev_points=None, ped_bev_valid_steps=None):
        ...
        with torch.no_grad():
            ...
            if is_inference and self._last_rgb_seq is not None and self._last_ego_seq is not None:
                ...
                if vlm_target_speed is not None or vlm_amplify_scale > 1.0:
                    ...
                    if vlm_amplify_scale > 1.0:
                        raw_traj_np = self.vlm.amplify_lateral_swerve_numpy(
                            raw_traj_np, scale=vlm_amplify_scale
                        )

                    if vlm_target_speed is not None:
                        raw_traj_np = self.vlm.resample_path_numpy(
                            raw_traj_np, vlm_target_speed, dt=0.5
                        )

                    resampled_tensor = torch.from_numpy(raw_traj_np).float().to(pred.device)
                    final_traj = final_traj.clone()
                    final_traj[0, :, :2] = resampled_tensor
                    resample_traj = final_traj.clone()
                    loss_dict['scenario'] = scenario
