...
class VLM_STP3_Gen(BaseVLM_STP3_Gen):
    ...
    def planning(self, *, bev_rgbs, trajs, gt_trajs, commands, target_points, occupancy=None):
        cfg = self.cfg
        aux_backup = {
            'LOSS_VIS_SENS_W': getattr(cfg, 'LOSS_VIS_SENS_W', 0.5),
            'LOSS_GATE_RISK_W': getattr(cfg, 'LOSS_GATE_RISK_W', 0.3),
            'LOSS_GATE_DROP_W': getattr(cfg, 'LOSS_GATE_DROP_W', 0.5),
            'LOSS_DY_CAP_W': getattr(cfg, 'LOSS_DY_CAP_W', 5.0),
            'LOSS_COARSE_W': getattr(cfg, 'LOSS_COARSE_W', 0.0),
            'LOSS_SMO_W': getattr(cfg, 'LOSS_SMO_W', 0.1),
            'LOSS_DIR_W': getattr(cfg, 'LOSS_DIR_W', 8.0),
        }

        cfg.LOSS_VIS_SENS_W = float(getattr(cfg, 'CLEAN_LOSS_VIS_SENS_W', 0.0))
        cfg.LOSS_GATE_RISK_W = float(getattr(cfg, 'CLEAN_LOSS_GATE_RISK_W', 0.0))
        cfg.LOSS_GATE_DROP_W = float(getattr(cfg, 'CLEAN_LOSS_GATE_DROP_W', 0.0))
        cfg.LOSS_DY_CAP_W = float(getattr(cfg, 'CLEAN_LOSS_DY_CAP_W', 0.0))
        cfg.LOSS_COARSE_W = float(getattr(cfg, 'CLEAN_LOSS_COARSE_W', 0.0))
        cfg.LOSS_SMO_W = float(getattr(cfg, 'CLEAN_LOSS_SMO_W', 0.05))
        cfg.LOSS_DIR_W = float(getattr(cfg, 'CLEAN_LOSS_DIR_W', 2.0))
        ...
        try:
            ...
            for key, value in aux_backup.items():
        ...
        if occupancy is not None:
            ...
            alpha_box_ar = float(getattr(self.cfg, 'LOSS_BOX_COL_AR_ALPHA', 0.0))
            lam_box = float(getattr(self.cfg, 'LOSS_BOX_COL_W', 2.0))
            box_coll = box_coll_pred

            if alpha_box_ar > 0.0:
                ar_xy = getattr(self.vlm, 'last_ar_xy', None)
                if ar_xy is not None:
                    box_coll_ar = self.box_collision_loss_soft(ar_xy, occupancy, return_per_t=False)
                    box_coll = box_coll + alpha_box_ar * box_coll_ar
        ...
        loss_dict['clean_loss_mode'] = torch.tensor(1.0, device=pred.device)
