...
class TrainingModule(pl.LightningModule):
    ...
    def _apply_finetune_trainable_mode(self):
        mode = str(getattr(self.cfg, 'PED_FINETUNE_TRAINABLE_MODE', 'ped_adapter'))
        self.ft_trainable_mode = mode

        if mode == 'ped_adapter':
            trainable_substrings = [
                'vlm.ped_risk_adapter',
            ]
            trainable_scalar_names = set()
        elif mode == 'ped_strict':
            trainable_substrings = [
                'vlm.ped_encoder',
            ...
            raise ValueError(
                ...
                "expected ped_adapter, ped_strict, ped_light, ped_only, or full_ft."
            )
    ...
    def on_train_epoch_start(self):
        if self.ft_trainable_mode == 'ped_adapter':
            # Keep every frozen BatchNorm/dropout layer on the baseline path
            # fixed while allowing only the new trajectory adapter to train.
            self.model.vlm.eval()
            self.model.vlm.ped_risk_adapter.train()
