...
# --- Metric 基底類別（Lightning 舊版 / 新版相容） ---
try:
    from pytorch_lightning.metrics.metric import Metric  # Lightning 1.x
except Exception:
    from torchmetrics.metric import Metric               # Lightning 2.x 搭配 torchmetrics

# --- stat_scores_multiple_classes 相容層 ---
# 先嘗試舊版路徑
try:
    from pytorch_lightning.metrics.functional.classification import stat_scores_multiple_classes  # Lightning 1.x 舊API
except Exception:
    # 再嘗試有些 torchmetrics 版本可能提供的路徑
    try:
        from torchmetrics.functional.classification import stat_scores_multiple_classes  # 若存在就直接用
    except Exception:
        # 最終方案：用類別型 API 實作同等功能（新版推薦）
        import torch
        from torchmetrics.classification import MulticlassStatScores

        def stat_scores_multiple_classes(preds, target, num_classes, average=None):
            """
            回傳 (tp, fp, tn, fn, support) 的 per-class 張量，等價於舊版 API。
            支援 preds 為 logits/probs (N,C,...) 或已為類別 index (N,...)
            """
            # 若 preds 是 logits/probs，先轉為類別 index
            if preds.ndim > target.ndim:
                try:
                    ch_dim = tuple(preds.shape).index(int(num_classes))
                    preds = preds.argmax(dim=ch_dim)
                except ValueError:
                    # 找不到就退回原先假設（dim=1）
            ...
            preds_f  = preds.reshape(-1).to(device).long()
            target_f = target.reshape(-1).to(device).long()

            C = int(num_classes)
            # 透過 bincount 建 CxC 混淆矩陣
            k = (target_f * C + preds_f)  # 將 (gt, pred) 映射到 [0, C*C)
            cm = torch.bincount(k, minlength=C*C).reshape(C, C)
...
# --- reduce 相容層 ---
try:
    from pytorch_lightning.metrics.functional.reduction import reduce  # 舊版
except Exception:
    from torchmetrics.utilities.data import dim_zero_mean as reduce     # 新版以 dim_zero_mean 取代
...
class IntersectionOverUnion(Metric):
    ...
    def compute(self):
        ...
        # return reduce(scores, reduction=self.reduction)    原本
    
        red = getattr(self, "reduction", "none")
        if red in (None, "none"):
            return scores                                 # 回傳每類別 IoU（evaluate.py / trainer.py 會取 scores[1]）
        elif red in ("elementwise_mean", "mean", "macro"):
            from torchmetrics.utilities.data import dim_zero_mean
            return dim_zero_mean(scores)                  # 對第一維（類別）取平均
        elif red == "sum":
            return scores.sum()
        else:
            # 不認得的字串就當平均
