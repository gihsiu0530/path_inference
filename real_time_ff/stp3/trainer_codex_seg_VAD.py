...
from stp3.model_vlm.codex_pure_VAD import VLM_STP3_Gen as VLM_STP3
...
class TrainingModule(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self._epoch_train_bev_teacher_distill_sum = 0.0
        self._epoch_train_bev_teacher_mask_ratio_sum = 0.0
        self._epoch_train_bev_visible_ratio_sum = 0.0
        self._epoch_train_bev_teacher_count = 0
        self._epoch_val_bev_teacher_distill_sum = 0.0
        self._epoch_val_bev_teacher_mask_ratio_sum = 0.0
        self._epoch_val_bev_visible_ratio_sum = 0.0
        self._epoch_val_bev_teacher_count = 0
    ...
    @staticmethod
    def _loss_scalar_to_float(value):
    ...
    def _safe_log_value(self, value, key=None):
        if torch.is_tensor(value):
            value = value.detach()
            if not torch.isfinite(value.float()).all():
                name = key if key is not None else "unknown"
                print(f"[non-finite log value] {name}")
    ...
    def _reset_epoch_bev_teacher_stats(self, prefix):
        setattr(self, f"_epoch_{prefix}_bev_teacher_distill_sum", 0.0)
        setattr(self, f"_epoch_{prefix}_bev_teacher_mask_ratio_sum", 0.0)
        setattr(self, f"_epoch_{prefix}_bev_visible_ratio_sum", 0.0)
        setattr(self, f"_epoch_{prefix}_bev_teacher_count", 0)

    def _accumulate_epoch_bev_teacher_stats(self, loss, prefix):
        if "bev_teacher_distill_log" not in loss:
            return
        distill = self._loss_scalar_to_float(loss["bev_teacher_distill_log"])
        mask_ratio = self._loss_scalar_to_float(loss.get("bev_teacher_mask_ratio_log", 0.0))
        visible_ratio = self._loss_scalar_to_float(loss.get("bev_visible_ratio_log", 0.0))

        setattr(
            self,
            f"_epoch_{prefix}_bev_teacher_distill_sum",
            getattr(self, f"_epoch_{prefix}_bev_teacher_distill_sum") + distill,
        )
        setattr(
            self,
            f"_epoch_{prefix}_bev_teacher_mask_ratio_sum",
            getattr(self, f"_epoch_{prefix}_bev_teacher_mask_ratio_sum") + mask_ratio,
        )
        setattr(
            self,
            f"_epoch_{prefix}_bev_visible_ratio_sum",
            getattr(self, f"_epoch_{prefix}_bev_visible_ratio_sum") + visible_ratio,
        )
        setattr(
            self,
            f"_epoch_{prefix}_bev_teacher_count",
            getattr(self, f"_epoch_{prefix}_bev_teacher_count") + 1,
        )

    def _log_epoch_bev_teacher_stats(self, prefix):
        count = getattr(self, f"_epoch_{prefix}_bev_teacher_count")
        if count <= 0:
            return

        distill_mean = getattr(self, f"_epoch_{prefix}_bev_teacher_distill_sum") / count
        mask_ratio_mean = getattr(self, f"_epoch_{prefix}_bev_teacher_mask_ratio_sum") / count
        visible_ratio_mean = getattr(self, f"_epoch_{prefix}_bev_visible_ratio_sum") / count
        gs = self.training_step_count

        self.logger.experiment.add_scalar(
            f"epoch_{prefix}_bev_teacher_distill_mean", distill_mean, global_step=gs
        )
        self.logger.experiment.add_scalar(
            f"epoch_{prefix}_bev_teacher_mask_ratio_mean", mask_ratio_mean, global_step=gs
        )
        self.logger.experiment.add_scalar(
            f"epoch_{prefix}_bev_visible_ratio_mean", visible_ratio_mean, global_step=gs
        )

        self.log(
            f"epoch_{prefix}_bev_teacher_distill_mean",
            torch.tensor(distill_mean, device=self.device),
            prog_bar=(prefix == "val"),
            on_epoch=True,
            logger=True,
            sync_dist=True,
        ...
        print(
            f"epoch_{prefix}_bev_teacher_distill_mean {distill_mean:.6f} "
            f"mask_ratio {mask_ratio_mean:.6f} visible_ratio {visible_ratio_mean:.6f}"
        )

    @staticmethod
    def _is_bev_warmup_param(name):
        return name.startswith("vlm.bev_input_proj.") or name.startswith("vlm.bev_encoder.")

    @staticmethod
    def _is_freeze_all_bev_param(name):
        exact_names = {
            "vlm.bev_scale_embed",
        }
        frozen_prefixes = (
        ...
        return name in exact_names or name.startswith(frozen_prefixes)

    @staticmethod
    def _is_freeze_all_bev_module(name):
        frozen_prefixes = (
            "vlm.bev_input_proj",
            "vlm.bev_encoder",
            "vlm.bev_obstacle_head",
        ...
        return any(name == prefix or name.startswith(prefix + ".") for prefix in frozen_prefixes)

    @staticmethod
    def _is_hard_decoder_warmup_trainable_param(name):
        exact_names = {
            "vlm.ego_query",
            "vlm.time_queries",
        }
        trainable_prefixes = (
            ...
            "vlm.traj_delta_head.",
            "vlm.endpoint_head.",
            "vlm.bev_residual_head.",
        )
        return name in exact_names or name.startswith(trainable_prefixes) or name.endswith("planning_weight")

    @staticmethod
    def _is_hard_decoder_warmup_frozen_module(name):
        frozen_prefixes = (
            "vlm.rgb_stem",
            "vlm.seg_rgb_stem",
            "vlm.seg_id_stem",
    ...
    def _hard_decoder_warmup_epochs(self):
        return int(getattr(self.cfg, "HARD_DECODER_WARMUP_EPOCHS",
                           getattr(self.cfg, "BEV_FREEZE_WARMUP_EPOCHS", 0)))

    def _set_hard_decoder_warmup_module_modes(self):
        freeze_all_bev = bool(getattr(self.cfg, "FREEZE_ALL_BEV", False))
        keep_perception_frozen = bool(getattr(self.cfg, "KEEP_PERCEPTION_FROZEN_AFTER_WARMUP", False))
        for name, module in self.model.named_modules():
            if freeze_all_bev and self._is_freeze_all_bev_module(name):
                module.eval()
                continue
            if self._is_hard_decoder_warmup_frozen_module(name):
                if self.current_epoch >= self._hard_decoder_warmup_epochs():
                    if not keep_perception_frozen:
                        continue
                    if name.startswith("vlm.bev_input_proj") or name.startswith("vlm.bev_encoder"):
                        continue
                    if name.startswith("vlm.bev_obstacle_head") or name.startswith("vlm.bev_teacher_adapter"):
                        continue
                module.eval()

    def _apply_hard_decoder_warmup(self):
        warmup_epochs = self._hard_decoder_warmup_epochs()
        hard_warmup = self.current_epoch < warmup_epochs
        freeze_all_bev = bool(getattr(self.cfg, "FREEZE_ALL_BEV", False))
        keep_perception_frozen = bool(getattr(self.cfg, "KEEP_PERCEPTION_FROZEN_AFTER_WARMUP", False))
        changed = 0
        for name, param in self.model.named_parameters():
            base_trainable = self._initial_requires_grad.get(name, param.requires_grad)
            if freeze_all_bev and self._is_freeze_all_bev_param(name):
                should_train = False
            elif hard_warmup:
                should_train = base_trainable and self._is_hard_decoder_warmup_trainable_param(name)
            elif keep_perception_frozen:
                should_train = base_trainable and (
                    self._is_hard_decoder_warmup_trainable_param(name)
                    or self._is_bev_warmup_param(name)
                    or name.startswith("vlm.bev_obstacle_head.")
                    or name.startswith("vlm.bev_teacher_adapter.")
                )
            else:
                should_train = base_trainable
            if param.requires_grad != should_train:
                changed += 1
            param.requires_grad_(should_train)

        self._set_hard_decoder_warmup_module_modes()

        if self.logger is not None:
            self.logger.experiment.add_scalar(
                "epoch_param_hard_decoder_warmup",
                1.0 if hard_warmup else 0.0,
                global_step=self.training_step_count,
            )
            self.logger.experiment.add_scalar(
                "epoch_param_freeze_all_bev",
                1.0 if freeze_all_bev else 0.0,
                global_step=self.training_step_count,
            )
        if hard_warmup:
            state = "hard decoder warmup"
        elif keep_perception_frozen:
            state = "bev+decoder finetune"
        else:
            state = "full finetune"
        print(f"[Hard warmup] epoch={self.current_epoch}, warmup_epochs={warmup_epochs}, mode={state}, freeze_all_bev={freeze_all_bev}, changed_tensors={changed}")

    
    def on_train_epoch_start(self):
        self._apply_hard_decoder_warmup()
        ...
        self._epoch_dy_exceed_sum = 0.0
        self._epoch_ratio_count = 0
        self._reset_epoch_bev_teacher_stats("train")
    ...
    def on_validation_epoch_start(self):
        ...
        self.val_cmd_fde_sum = {"FORWARD": 0.0, "LEFT": 0.0, "RIGHT": 0.0}
        self.val_cmd_count = {"FORWARD": 0, "LEFT": 0, "RIGHT": 0}
        self._reset_epoch_bev_teacher_stats("val")
    ...
    def shared_step(self, batch, is_train):
        ...
        # Metrics
            ...
            if self.cfg.PLANNING.ENABLED:
                ...
                present_occupancy = torch.logical_or(
                    labels['segmentation'][:, n_present - 1].squeeze(1),
                    labels['pedestrian'][:, n_present - 1].squeeze(1),
                )
                ...
                if bool(getattr(self.cfg, "NAN_DEBUG", False)) and not torch.isfinite(final_traj).all():
                    finite = torch.isfinite(final_traj)
                    msg = (
                        "[NaN debug] validation final_traj is non-finite before metric "
    ...
    def training_step(self, batch, batch_idx):
        ...
        for key, value in loss.items():
            self.logger.experiment.add_scalar(
                ...
                self._safe_log_value(value, key),
                global_step=self.training_step_count,
            )
        self._accumulate_epoch_bev_teacher_stats(loss, "train")
        ...
        loss_total = loss["planning"]
        if not torch.isfinite(loss_total):
            print("[NaN debug] non-finite training loss; dumping trainable parameter stats")
            for n, p in self.model.named_parameters():
                if p.requires_grad and not torch.isfinite(p).all():
                    finite = torch.isfinite(p)
                    print(f"  nonfinite param: {n} ratio={float((~finite).float().mean().item()):.6f}")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self._accumulate_epoch_bev_teacher_stats(loss, "val")
    ...
    def on_train_epoch_end(self):
        ...
        self._log_epoch_bev_teacher_stats("train")
    ...
    def on_validation_epoch_end(self):
        ...
        self._log_epoch_bev_teacher_stats("val")
    ...
    def configure_optimizers(self):
        ...
        freeze_all_bev = bool(getattr(self.cfg, "FREEZE_ALL_BEV", False))
        for n, p in self.model.named_parameters():
            if freeze_all_bev and self._is_freeze_all_bev_param(n):
            ...
            if ("vlm.clip.visual.transformer.resblocks" in n) or ("vlm.clip.visual.ln_" in n):
                clip_last_params.append(p)
            elif self._is_bev_warmup_param(n):
                bev_params.append(p)
        ...
        bev_lr_mult = float(getattr(self.cfg, "BEV_LR_MULT", 0.1))
        bev_lr = head_lr * bev_lr_mult
        wd      = self.cfg.OPTIMIZER.WEIGHT_DECAY
        # clip_lr = 0.1
        ...
        max_lrs = [head_lr]
        if len(bev_params) > 0:
            print("bev_lr : ", bev_lr, "bev_lr_mult : ", bev_lr_mult)
            param_groups.append({"params": bev_params, "lr": bev_lr, "weight_decay": wd})
            max_lrs.append(bev_lr)
        else:
            print("bev_lr : skipped (no BEV warmup params)")
