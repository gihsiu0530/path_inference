...
class VLM_Generative(nn.Module):
    ...
    def __init__(self, clip_name: str, device: torch.device, n_future: int, input_size: int = 224,
                 prompts: Dict[str, str] = {
                     "LEFT":    "turn left, stay on drivable road, avoid obstacles, smooth path, keep current speed",
                     "FORWARD": "go straight, stay on drivable road, avoid obstacles, smooth path, keep current speed",
                     "RIGHT":   "turn right, stay on drivable road, avoid obstacles, smooth path, keep current speed",
                 }):
        super().__init__()
        ...
        # === Speed / Timing head + prompt conditioning ===
        self.speed_head = nn.Sequential(
            nn.Linear(self.vis_width, self.vis_width),
            nn.GELU(),
            nn.Linear(self.vis_width, 1)  # 每步輸出一個 Δs_raw
        )

        # 文字投影到速度通道（與 txt_to_vis 分開，避免干擾幾何）
        self.txt_to_speed = nn.Linear(self.vis_width, self.vis_width, bias=False)

        # 初始「保持速度」：gate=0 表示用 GT/等距的 Δs；之後你把它拉高（或改為由 prompt 決定）
        self.register_buffer("speed_gate", torch.tensor(0.0))   # tensor，方便日後 .fill_(x)
        self.min_step = 1e-3  # re-timing 的最小步幅，避免 s_t 卡死
    ...
    def _retime_by_deltas(self, xy_geom: torch.Tensor, deltas: torch.Tensor) -> torch.Tensor:
        """
        xy_geom: (B,T,2) 幾何路徑（不含速度資訊）
        deltas : (B,T,1) 非負「步進量」(Δs_raw 經正規化)，行和為 1
        回傳   : (B,T,2) 依據 deltas 的累積 s_t 對 xy_geom 做弧長重採樣後的軌跡
        """
        device = xy_geom.device
        B, T, _ = xy_geom.shape

        # 1) 幾何路徑的弧長座標 ŝ ∈ [0,1]
        #    arc[b,0]=0； arc[b,t]=Σ||xy[t]-xy[t-1]||
        step = torch.zeros(B, T, device=device)
        step[:, 1:] = (xy_geom[:, 1:] - xy_geom[:, :-1]).norm(dim=-1)
        arc = torch.cumsum(step, dim=1)  # (B,T)
        total = arc[:, -1].clamp_min(1e-6).unsqueeze(-1)   # (B,1)
        s_hat = arc / total                                 # (B,T) in [0,1]

        # 2) 由 deltas 得到 s_t：行和為 1，再累積
        deltas = deltas.clamp_min(self.min_step)
        deltas = deltas / (deltas.sum(dim=1, keepdim=True) + 1e-6)   # 重新歸一
        s = torch.cumsum(deltas.squeeze(-1), dim=1)                  # (B,T)
        s = torch.clamp(s, 0.0, 1.0)

        # 3) 在 s_hat 上作線性插值，找對應點
        #    對每個 s[b,t] 找到 s_hat[b,i] <= s[b,t] < s_hat[b,i+1]
        #    並做線性內插
        #    為避免 Python 迴圈，這裡用 torch.searchsorted 處理每個 batch
        s_hat_cpu = s_hat.detach().cpu()   # searchsorted 目前 CPU 較穩
        idx = torch.searchsorted(s_hat_cpu, s.detach().cpu(), right=False)  # (B,T)
        idx = idx.clamp(min=1, max=T-1).to(device)

        s0 = s_hat.gather(1, idx-1)
        s1 = s_hat.gather(1, idx)
        denom = (s1 - s0).clamp_min(1e-6)
        w = (s - s0) / denom  # (B,T)

        # 對應的兩端點座標
        i0 = (idx-1).unsqueeze(-1).expand(B, T, 2)
        i1 = idx.unsqueeze(-1).expand(B, T, 2)
        p0 = torch.gather(xy_geom, 1, i0)
        p1 = torch.gather(xy_geom, 1, i1)

        p = p0 + w.unsqueeze(-1) * (p1 - p0)  # (B,T,2)
        return p
    ...
    def generate_autoregressive(
        ...
        *,
        ...
        # === 幾何路徑（不含時間）：xy_geom ===
        xy_geom = xy  # (B,T,2) 這是 AR + coarse 殘差後的幾何

        # === 目標/基準 Δs_GT（保持速度用；沒有 GT 就等距）===
        if gt_trajs is not None and gt_trajs.size(-1) >= 2:
            gt_xy = gt_trajs[..., :2]
            gt_step = torch.zeros(B, T, device=device)
            gt_step[:, 1:] = (gt_xy[:, 1:] - gt_xy[:, :-1]).norm(dim=-1)
            deltas_gt = gt_step.unsqueeze(-1)                     # (B,T,1)
        else:
            deltas_gt = torch.ones(B, T, 1, device=device)        # 等距

        deltas_gt = deltas_gt.clamp_min(self.min_step)
        deltas_gt = deltas_gt / (deltas_gt.sum(dim=1, keepdim=True) + 1e-6)  # 行和=1

        # === 速度頭：Δs_pred ===
        speed_ctx = fused + self.txt_to_speed(text_vis).unsqueeze(1)          # (B,T,C)
        deltas_pred_raw = self.speed_head(speed_ctx)                           # (B,T,1)
        deltas_pred = F.softplus(deltas_pred_raw) + self.min_step
        deltas_pred = deltas_pred / (deltas_pred.sum(dim=1, keepdim=True) + 1e-6)

        # === gate 混合 ===
        g = float(self.speed_gate.item())     # [0,1]
        deltas_mix = (1.0 - g) * deltas_gt + g * deltas_pred

        # === re-timing ===
        xy_final = self._retime_by_deltas(xy_geom, deltas_mix)                 # (B,T,2)

        # 封裝兩條軌跡與一些監控資訊
        z  = torch.zeros(B, T, 1, device=device)
        traj_geom  = torch.cat([xy_geom,  z], dim=-1)  # (B,T,3) 固定時間格 → 用於「公平評估」
        traj_final = torch.cat([xy_final, z], dim=-1)  # (B,T,3) re-timing 後 → 用於「行為落地/部署」

        return {
            "traj_geom":  traj_geom,
            "traj_final": traj_final,
            "deltas_pred": deltas_pred.detach(),   # 方便 log
            "deltas_gt":   deltas_gt.detach(),     # 方便 log
        }
...
class VLM_STP3_Gen(nn.Module):
    ...
    def planning(self, *, bev_rgbs, trajs, gt_trajs, commands, target_points, occupancy=None):
        ...
        pred  = outs["traj_geom"]     # (B,T,3)
        pred_with_speed = outs["traj_final"]    # (B,T,3)
        ...
        bad = []
        for name, t in dbg_terms.items():
            if not torch.isfinite(t):
                bad.append(name)
                # 輸出一些範圍資訊，避免 .min()/.max() 被 NaN 卡住
                tt = t.detach()
                m = torch.isfinite(tt)
                if m.any():
                    print(f"[PLANNING NaN] {name} finite-range:",
                        f"min={tt[m].min().item():.6g}, max={tt[m].max().item():.6g}")
                else:
                    print(f"[PLANNING NaN] {name} has no finite values")

        # 進一步檢查幾個最可能的源頭
        if bad:
            # 幾何軌跡 / 速度重定時的 delta 是否正常
            try:
                print("  xy_geom finite:", torch.isfinite(pred[..., :2]).all().item(),
                    "xy_final finite:", torch.isfinite(pred_with_speed[..., :2]).all().item())
            except Exception:
                pass
            try:
                print("  deltas_pred finite:",
                    torch.isfinite(outs.get('deltas_pred', torch.tensor([0.0], device=device))).all().item())
            except Exception:
                pass
            # 也把一些數值範圍印出來（只印有限值的 min/max）
            def _pp(label, x):
                if torch.is_tensor(x):
                    xx = x.detach()
                    m = torch.isfinite(xx)
                    if m.any():
                        print(f"  {label} min={xx[m].min().item():.6g}, max={xx[m].max().item():.6g}")
            _pp("xy_geom", pred[..., :2])
            _pp("xy_final", pred_with_speed[..., :2])
            _pp("deltas_pred", outs.get('deltas_pred', None))
            _pp("deltas_gt",   outs.get('deltas_gt', None))
        ...
        return loss, pred, pred_with_speed, torch.tensor(0.0, device=device), torch.tensor(0.0, device=device)
