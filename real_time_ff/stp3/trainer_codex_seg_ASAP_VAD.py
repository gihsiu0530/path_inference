...
from stp3.model_vlm.codex_pure_ASAP_VAD import VLM_STP3_Gen as VLM_STP3
...
class TrainingModule(pl.LightningModule):
    ...
    def on_train_epoch_start(self):
        vad_vector_base_scale = float(getattr(self.cfg, "VAD_VECTOR_CONTEXT_SCALE", 0.0))
        vad_vector_warmup = float(getattr(self.cfg, "VAD_VECTOR_CONTEXT_WARMUP_EPOCHS", 0.0))
        vad_vector_ramp = max(float(getattr(self.cfg, "VAD_VECTOR_CONTEXT_RAMP_EPOCHS", 1.0)), 1e-6)
        if self.current_epoch < vad_vector_warmup:
            vad_vector_scale = 0.0
        else:
            vad_vector_scale = vad_vector_base_scale * min(
                1.0,
                (self.current_epoch + 1.0 - vad_vector_warmup) / vad_vector_ramp,
            )
        self.model.vlm.vad_vector_context_scale = vad_vector_scale
        ...
        if self.logger is not None:
            ...
            self.logger.experiment.add_scalar(
                "epoch_param_vad_vector_context_scale",
                vad_vector_scale,
                global_step=self.training_step_count,
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_total = loss.get("planning")
        if loss_total is None:
            train_terms = [v for k, v in loss.items() if not k.endswith("_log")]
            loss_total = sum(train_terms) if train_terms else sum(loss.values())
