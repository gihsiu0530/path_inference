...
from stp3.model_vlm.codex_re_thinking import VLM_STP3_Gen as VLM_STP3
...
class TrainingModule(pl.LightningModule):
    ...
    def on_train_epoch_start(self):
        ...
        if hasattr(self.model, "vlm") and hasattr(self.model.vlm, "AR_TF_RATIO"):
        ...
        if is_train:
            ...
            if self.cfg.PLANNING.ENABLED:
                ...
                pl_loss, final_traj, resample_traj, loss_rank, loss_dict = self.model.planning(
                    ...
                    gt_trajs=batch.get('admlp_gt', labels['gt_trajectory']),
                    commands=command,
                    target_points=target_points,
                    occupancy=occupancy,
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_total = sum(value for key, value in loss.items() if not key.endswith("_log"))
    ...
    def configure_optimizers(self):
        params = [
            ...
            if p.requires_grad and not n.endswith("planning_weight")
        ]
        assert len(params) > 0, "找不到可訓練 AD-MLP 參數"

        lr = float(getattr(self.cfg, "ADMLP_LR", 1e-4))   #4e-6
        wd = float(getattr(self.cfg, "ADMLP_WEIGHT_DECAY", 1e-2))

        print("admlp_lr : ", lr)
        print("admlp_weight_decay : ", wd)
