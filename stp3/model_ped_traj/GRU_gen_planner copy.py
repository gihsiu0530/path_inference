...
class VLM_Generative(nn.Module):
    ...
    def generate_autoregressive(
        ...
        *,
        ...
        for t in range(T):
            ctx_t = fused[:, t, :]                     # (B,C)
            embedded_xy = self.xy_embedder(prev_xy)    # (B,C)
...
class VLM_STP3_Gen(nn.Module):
    ...
    def occupancy_collision_rate(self, trajs_xy: torch.Tensor, occupancy: torch.Tensor) -> torch.Tensor:
        ...
        assert occupancy is not None, "occupancy None：trainer 沒有把佔據圖傳進來"
    ...
    def planning(self, *, bev_rgbs, trajs, gt_trajs, commands, target_points, occupancy=None):
        ...
        # print("[DBG] occupancy>0 ratio:", occupancy.float().mean().item())
        ...
        lam_smo = getattr(self.cfg, 'LOSS_SMO_W', 0.6)
        lam_vel = getattr(self.cfg, 'LOSS_VEL_W', 0.9)   # 新增：速度監督
