...
class VLM_Generative(nn.Module):
    ...
    def __init__(self, clip_name: str, device: torch.device, n_future: int, input_size: int = 224,
                 prompts: Dict[str, str] = {
                     ...
                     # === 新增這兩個 ===
                    "SLOW":        "slow down, drive carefully, dense trajectory points, reduce speed",
                    "STOP":        "stop, brake, hold position, stationary, do not move",   
                 }):
        super().__init__()
...
class VLM_STP3_Gen(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.black_image_test = False   # ← 開啟影像消融測試

        print("model : gemini_autocast")
    ...
    def forward(self, image, intrinsics, extrinsics, future_egomotion, *, rgb_224_seq, seg_224_seq):
        ...
        if self.black_image_test:
            print("沒影像測試！！")
            self._last_rgb_seq = torch.zeros_like(rgb_224_seq, device=device)
            self._last_seg_seq = torch.zeros_like(seg_224_seq, device=device)
        else:
    ...
    def planning(self, *, bev_rgbs, trajs, gt_trajs, commands, target_points, occupancy=None):
        ...
        lam_coarse = getattr(self.cfg, 'LOSS_COARSE_W', 0.2)
        ...
        return loss, pred, final_traj, torch.tensor(0.0, device=device), torch.tensor(0.0, device=device)
