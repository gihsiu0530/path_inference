...
class DepthTokenEncoder(nn.Module):
    """
    將 (B,1,224,224) 深度圖 → (B,S,C) tokens
    - 先用 Conv2d stride=16 的「ViT 風格貼片化」取 14x14=196 個 tokens
    - 再用 LayerNorm + GELU 做簡單的局部建模
    - 輸出通道 C = vis_width（與 CLIP tokens 對齊）
    """
    def __init__(self, out_dim: int, patch_size: int = 16):
        super().__init__()
        self.patch = nn.Conv2d(1, out_dim, kernel_size=patch_size, stride=patch_size, padding=0, bias=False)  # (B,out,14,14)
    ...
    def forward(self, d: torch.Tensor) -> torch.Tensor:
        # d: (B,1,224,224), float32/float16
        x = self.patch(d)                       # (B,C,14,14)
        x = x.flatten(2).transpose(1, 2)        # (B,196,C)
        x = self.post(x)                        # (B,196,C)
        # L2 normalize（與 CLIP tokens 行為一致）
...
class VLM_Generative(nn.Module):
    ...
    def __init__(self, clip_name: str, device: torch.device, n_future: int, input_size: int = 224,
        ...
        self.dec_in_ln  = nn.LayerNorm(self.vis_width * 2)
                ...
                # === 深度模態編碼器 ===
        self.depth_enc = DepthTokenEncoder(self.vis_width, patch_size=16)

        # 深度數值正規化（把任意量級壓回穩定區間）
        self.depth_affine = nn.Parameter(torch.tensor([1.0, 0.0]))  # y = a * x + b
        self.depth_clip_min = nn.Parameter(torch.tensor(0.0), requires_grad=False)
        self.depth_clip_max = nn.Parameter(torch.tensor(50.0), requires_grad=False)  # 依場景可改 30~80

        # 融合閘門：學習多少信任 DEPTH 與 RGB/SEG
        ...
        # ★ 新增：深度數值轉換模式（'inv' 或 'log'；預設 inv 對近距離最友善）
        self.depth_mode = getattr(self, "DEPTH_MODE", "inv")
        # ★ 新增：多相機深度縮減策略（'mean' 或 'concat'；預設 mean 減少 token 稀釋）
        self.depth_reduce = getattr(self, "DEPTH_REDUCE", "mean")
    ...
    def _depth_token_weights_from_raw(self, depth_bt: torch.Tensor) -> torch.Tensor:
        """
        給一幀原始深度 (B,1,224,224)，輸出對應 depth tokens 的權重 (B,Sd,1)。
        作法：
          1) 反深度或對數深度，強調近距離
          2) AvgPool 16×16 → 14×14（貼片對齊），再 normalize 到 [0,1]
        """
        if depth_bt.dim() == 3:
            depth_bt = depth_bt.unsqueeze(1)
        d = depth_bt.clamp(min=self.depth_clip_min.item(), max=self.depth_clip_max.item())
        eps = 1e-3
        if str(self.depth_mode).lower() == "inv":
            d = 1.0 / (d + eps)                # 近距離大、遠距離小
        else:
            d = torch.log1p(d)                 # 對數，拉開 1~10m
        # 取貼片平均（對齊 DepthTokenEncoder 的 patch_size=16）
        w = F.avg_pool2d(d, kernel_size=16, stride=16)   # (B,1,14,14)
        w = w.flatten(2).transpose(1, 2)                 # (B,196,1)
        # normalize 到 [0,1]
        w_min = w.amin(dim=1, keepdim=True)
        w_max = w.amax(dim=1, keepdim=True)
        w = (w - w_min) / (w_max - w_min + 1e-6)
        # 稍微壓一點動態（0.6~1.0），避免過度影響
        w = 0.6 + 0.4 * w
        return w
    ...
    def generate_autoregressive(
        ...
        *,
        ...
        vis_tokens = self.build_vis_tokens(rgb_seq, seg_seq, ego_seq,
                                        depth_seq=getattr(self, "_last_depth_seq_bt", None))  # (B,S,C)
    ...
    def build_vis_tokens(self, rgb_seq: torch.Tensor, seg_seq: torch.Tensor,
                         ego_seq: torch.Tensor, depth_seq: torch.Tensor = None) -> torch.Tensor:
        """
        輸入：
          - rgb_seq: (B, T_rf, 3, 224, 224) 或 (B,T_rf,224,224,3) 皆可
          - seg_seq: (B, T_rf, 3, 224, 224) 或 (B,T_rf,224,224,3)
          - depth_seq: (B, T_rf, N, 1, 224, 224) 或 (B, T_rf, 1, 224, 224)；會自動展平到 (B,T_rf,1,224,224)
          - ego_seq: (B, T_rf, 4)
        輸出：
          - vis_tokens: (B, T_rf*(S_rgb+S_seg+S_dep), C)
        """
        B, T_rf = rgb_seq.shape[:2]
        all_tokens = []
        ...
        # 若有多相機深度 (N>1)，先在 N 維做平均或 concat。這裡先用 concat（資訊最多）。
        use_depth = (depth_seq is not None)
        if use_depth:
            if depth_seq.dim() == 6:
                raise RuntimeError("depth為啥事六為")
                # (B,T,N,1,H,W) → 合併 N 到 batch 再還原
                B0, T0, N, _, H, W = depth_seq.shape
                depth_merged = depth_seq.reshape(B0*T0*N, 1, H, W)
                # 預處理 & 編碼
                d_proc = self._preprocess_depth_tensor(depth_merged)          # (B*T*N,1,224,224)
                d_tok  = self.depth_enc(d_proc)                               # (B*T*N, Sd, C)
                Sd, C = d_tok.shape[1], d_tok.shape[2]
                d_tok = d_tok.reshape(B0, T0, N*Sd, C)                         # (B,T,N*Sd,C)
            elif depth_seq.dim() == 5:
                # (B,T,1,H,W)
                B0, T0, _, H, W = depth_seq.shape
                d_proc = self._preprocess_depth_tensor(depth_seq.reshape(B0*T0, 1, H, W))
                d_tok  = self.depth_enc(d_proc).reshape(B0, T0, -1, self.vis_width)  # (B,T,Sd,C)
            else:
                raise ValueError("depth_seq 維度不支援")

        for t in range(T_rf):
            rgb_t = self._preprocess_clip_tensor(rgb_seq[:, t])    # (B,3,224,224) → norm
            seg_t = self._preprocess_clip_tensor(seg_seq[:, t])

            tok_rgb = self._encode_tokens(rgb_t)                   # (B,Sr,C)
            tok_seg = self._encode_tokens(seg_t)                   # (B,Ss,C)

            if use_depth:
                tok_dep = d_tok[:, t]                              # (B,Sd*,C)
                # 幾何權重：對應貼片平均的「反深度/對數深度」→ 近距離權重較大
                if depth_seq.dim() == 6 and self.depth_reduce != "mean":
                    # concat 模式時，這裡就不容易對應；建議仍使用 mean
                    pass
                else:
                    # (B,1,224,224) 單相機/均值後深度
                    raw_depth_t = depth_seq[:, t]                  # (B,1,224,224)
                    w_tok = self._depth_token_weights_from_raw(raw_depth_t)  # (B,Sd,1)
                    tok_dep = tok_dep * w_tok                      # 幾何引導

                # 閘門融合：加下限，避免學成 0
                ctx = torch.cat([tok_rgb.mean(dim=1), tok_seg.mean(dim=1)], dim=-1)
                ctx = F.linear(ctx, weight=torch.eye(ctx.shape[-1], device=ctx.device)[:, :self.vis_width])
                lam = self.gate_depth(ctx).unsqueeze(1)            # (B,1,1), 0~1
                lam = 0.5 + 0.5 * lam                              # 下限 0.5
                tok_dep = lam * tok_dep
                tok = torch.cat([tok_rgb, tok_seg, tok_dep], dim=1)
            else:
                tok = torch.cat([tok_rgb, tok_seg], dim=1)

            ego_embed = self.ego_mlp(ego_seq[:, t].to(self.device))  # (B,C)
            tok = tok + ego_embed.unsqueeze(1)                       # 廣播到 token 維
            all_tokens.append(tok)

        vis_tokens = torch.cat(all_tokens, dim=1)  # (B, T_rf*(S_all), C)
        return vis_tokens
    ...
    def _preprocess_depth_tensor(self, depth_bt: torch.Tensor) -> torch.Tensor:
        """
        depth_bt: (B,1,H,W) 或 (B,H,W) 的深度（float）
        輸出 clamp + 仿射縮放後的 (B,1,224,224) float32
        """
        if depth_bt.dim() == 3:
            depth_bt = depth_bt.unsqueeze(1)  # → (B,1,H,W)
        # 尺寸對齊
        if depth_bt.shape[-1] != self.input_size or depth_bt.shape[-2] != self.input_size:
            depth_bt = F.interpolate(depth_bt, size=(self.input_size, self.input_size),
                                     mode="bilinear", align_corners=False)
        # 夾取 + 仿射（避免極端值）
        d = depth_bt
        d = torch.clamp(d, min=self.depth_clip_min.item(), max=self.depth_clip_max.item())
        a, b = self.depth_affine[0].float(), self.depth_affine[1].float()
        d = a * d + b
        # 轉回 float32 計算較穩（可依需要改成 fp16）
...
class VLM_STP3_Gen(nn.Module):
    ...
    def forward(self, image, intrinsics, extrinsics, future_egomotion, *, rgb_224_seq, seg_224_seq, depth_224_seq=None):
        ...
        self._last_seg_seq = seg_224_seq.to(device, non_blocking=True).to(torch.float16)
                ...
                # depth_224_seq 允許兩種形狀：
        # (B, T_rf, 1, 224, 224)  或  (B, T_rf, N, 1, 224, 224)
        if depth_224_seq is not None:
            if depth_224_seq.dim() == 5:
                self._last_depth_seq_bt = depth_224_seq.to(device, non_blocking=True).to(torch.float32)  # (B,T,1,224,224)
            elif depth_224_seq.dim() == 6:
                self._last_depth_seq_bt = depth_224_seq.to(device, non_blocking=True).to(torch.float32)  # (B,T,N,1,224,224)
            else:
                raise ValueError("depth_224_seq 維度不支援")
        else:
            self._last_depth_seq_bt = None
