...
class VLM_Generative(nn.Module):
    ...
    def make_coarse_baseline(self, ego_seq: torch.Tensor, T: int = None) -> torch.Tensor:
        ...
        # T_rf=3: [t-1.0s, t-0.5s, t]，且最後一幀(t)被 wrapper 設為 0
        # 用 -3 與 -2 兩步平均，降低單步噪聲/起步煞車瞬態
        dxy_norm = 0.5 * (ego_seq[:, -3, 0:2] + ego_seq[:, -2, 0:2])  # (B,2)
...
class VLM_STP3_Gen(nn.Module):
    ...
    def planning(self, *, bev_rgbs, trajs, gt_trajs, commands, target_points, occupancy=None):
        ...
        if occupancy is not None:
            ...
            risk_pow  = float(getattr(self.cfg, "RISK_POW", 2.0))          # 先用 1~2 比較穩
            risk_mode = str(getattr(self.cfg, "RISK_REDUCE", "topk"))      # "topk" / "max" / "mean"
            topk_frac = float(getattr(self.cfg, "RISK_TOPK_FRAC", 0.2))    # 0.2 常用

            def reduce_risk(risk_per_t: torch.Tensor) -> torch.Tensor:
                # risk_per_t: (B,T)
                ...
                if risk_mode == "mean":
                    ...
                    topk_vals, _ = torch.topk(r, k=k, dim=1, largest=True, sorted=False)  # (B,k)
                    return topk_vals.mean()
                else:
                    raise ValueError(f"Unknown RISK_REDUCE={risk_mode}")

            coll_final = reduce_risk(risk_final_t)
            coll_ar    = reduce_risk(risk_ar_t)

            # 4) 合併：final + α * AR
            alpha_ar = float(getattr(self.cfg, "LOSS_COL_AR_ALPHA", 1.0))  # 建議先 1.0，再試 2.0
        ...
        loss_dict["risk_mask_ratio"] = (mask.float().mean()).detach()
