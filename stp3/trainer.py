...
from stp3.model_vlm.image_forced import VLM_STP3_Gen as VLM_STP3


# from stp3.model_vlm.GRU_gen_planner import VLM_STP3_Gen as VLM_STP3
...
class TrainingModule(pl.LightningModule):
    ...
    def shared_step(self, batch, is_train):
        ...
        if is_train:
            ...
            if self.cfg.PLANNING.ENABLED:
                ...
                pl_loss, final_traj, resample_traj, loss_rank, loss_cost = self.model.planning(
                    bev_rgbs=bev_rgbs,
        ...
        # Metrics
            ...
            if self.cfg.PLANNING.ENABLED:
                ...
                _, final_traj, resample_traj, loss_rank, loss_cost = self.model.planning(
                    bev_rgbs=bev_rgbs,
                    # trajs=trajs[:, :, 1:],
                    trajs= trajs,
                    gt_trajs=labels['gt_trajectory'][:, 1:],
    ...
    def configure_optimizers(self):
        ...
        # === OneCycleLR ===
        # -------------直接用看的，有改資料集記得改-----------
        # steps_per_epoch = 1376    2923   5505
