...
class CustomCampusDataset(torch.utils.data.Dataset):
    ...
    BAD_TIMESTAMPS = {1779090675484601027}
    ...
    def __init__(self, cfg, is_train=False):
        ...
        if len(self.df) >= self.sequence_length:
            ...
            for i in range(len(self.df) - self.sequence_length + 1):
                ...
                seq_indices = list(range(i, i + self.sequence_length))
                if self._sequence_has_bad_timestamp(seq_indices):
                    skipped_bad_timestamp += 1
                    continue
                self.indices.append(seq_indices)
            if skipped_bad_timestamp > 0:
                print(
                    f"[CustomCampusDataset] skipped {skipped_bad_timestamp} sequences containing bad timestamp(s): "
                    f"{sorted(self.BAD_TIMESTAMPS)}"
                )
        else:
    ...
    def _row_has_bad_timestamp(self, row) -> bool:
        for col in self.BAD_TIMESTAMP_COLUMNS:
            if col in row and not pd.isna(row[col]):
                try:
                    if int(row[col]) in self.BAD_TIMESTAMPS:
                        return True
                except (TypeError, ValueError):
                    pass
        if "filename" in row and not pd.isna(row["filename"]):
            stem = os.path.splitext(os.path.basename(str(row["filename"])))[0]
            try:
                return int(stem) in self.BAD_TIMESTAMPS
            except ValueError:
                return False
        return False

    def _sequence_has_bad_timestamp(self, seq_indices) -> bool:
        return any(self._row_has_bad_timestamp(self.df.iloc[idx]) for idx in seq_indices)
