...
def visualise_output(labels, output, cfg):
    ...
    # ---- 取得 GT 解析度，先把 pred logits 對齊到 GT 大小 ----
    H_t, W_t = labels['segmentation'].shape[-2], labels['segmentation'].shape[-1]

    def _resize_logits_if_needed(logits):
        # logits: (B, S, C, H, W)
        if logits.shape[-2:] != (H_t, W_t):
            bs, s, c, h, w = logits.shape
            logits = F.interpolate(
                logits.flatten(0,1), size=(H_t, W_t),
                mode="bilinear", align_corners=False
            ).unflatten(0, (bs, s))
        return logits

    # 只要之後會拿來 argmax 成語意圖或做可視化的，都對齊
    if 'segmentation' in output:
        output['segmentation'] = _resize_logits_if_needed(output['segmentation'])
    if cfg.SEMANTIC_SEG.PEDESTRIAN.ENABLED and ('pedestrian' in output):
        output['pedestrian'] = _resize_logits_if_needed(output['pedestrian'])
    ...
    for t in range(sequence_length):
        ...
        # Predictions
        # 1. 修改：只有當 output 有 segmentation 時才畫，否則畫全黑或用 GT 充數
        if 'segmentation' in output:
            semantic_seg = output['segmentation'].argmax(dim=2).detach().cpu().numpy()
            semantic_plot = semantic_colours[semantic_seg[b, t][::-1, ::-1]]
            semantic_plot = make_contour(semantic_plot)
        else:
            # 如果模型沒輸出分割，就用全黑代替，避免報錯
            semantic_plot = np.zeros((H_t, W_t, 3), dtype=np.uint8) 
            semantic_plot = make_contour(semantic_plot)
        ...
        # Instance Seg (略，照舊，它原本就有防呆)
        if cfg.INSTANCE_SEG.ENABLED and 'instance_center' in output:
            # ... (保留原本代碼) ...
            pass # 這裡太長省略，若你沒開 Instance 功能不用改
        else:
            instance_plot = np.zeros_like(semantic_plot)
            center_plot = np.zeros_like(semantic_plot)
            offset_plot = np.zeros_like(semantic_plot)
        ...
        # Flow (略)
        if cfg.INSTANCE_FLOW.ENABLED and 'instance_flow' in output:
            # ...
            pass
        else:
            future_flow_plot = np.zeros_like(semantic_plot)

        # 3. ★重點修改：強制畫軌跡 (Planning)
        # 邏輯：即使沒預測 HD Map，只要有 'selected_traj' 就畫
        if 'selected_traj' in output:
            # 技巧：因為我們沒有預測 HD Map，所以借用 "GT 的 HD Map" (labels['hdmap']) 當背景
            # 這樣你才能看到你的綠色軌跡線 (output['selected_traj']) 是否在道路上
            
            # labels['hdmap'][b] 是真實地圖
            # output['selected_traj'][b] 是你的預測軌跡
            planning_plot = plot_planning(labels['hdmap'][b], output['selected_traj'][b], cfg)
            planning_plot = make_contour(planning_plot)
        elif cfg.SEMANTIC_SEG.HDMAP.ENABLED and 'hdmap' in output:
            # 原本的邏輯 (如果有預測 HD Map)
