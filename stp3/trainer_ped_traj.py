...
from stp3.config_ped_traj import get_cfg
...
from stp3.model_ped_traj.image_forced_ped_traj_codex import VLM_STP3_Gen as VLM_STP3
...
class TrainingModule(pl.LightningModule):
            ...
            if self.cfg.PLANNING.ENABLED:
                ...
                if ped_total > 0:
                    ...
                    for key, value in ped_scores.items():
                        self.logger.experiment.add_scalar(
                            'epoch_val_plan_ped_only_on_ped_traj_' + key,
                            value.mean(),
                            global_step=self.training_step_count,
                        )
    ...
    def configure_optimizers(self):
        ...
        # === OneCycleLR ===
        estimated_total_steps = int(getattr(self.trainer, 'estimated_stepping_batches', 0) or 0)
        if estimated_total_steps > 0:
            total_steps = estimated_total_steps
            steps_per_epoch = max(1, math.ceil(total_steps / max(1, self.cfg.EPOCHS)))
        else:
            steps_per_epoch = 5505
