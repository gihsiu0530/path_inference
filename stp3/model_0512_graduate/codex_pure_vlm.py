...
class ClipTrafficSignGate:
    ...
    DEFAULT_PROMPTS: Dict[str, List[str]] = {
        "NORMAL": [
            "a normal flat asphalt road without a raised obstacle",
            "a clear road surface with no physical speed bump",
            "a normal driving scene on a flat road",
            "a curved road or intersection without any foreground road hump",
        ],
        "YELLOW_LANE_MARKING": [
            "yellow center lane lines painted on a flat asphalt road",
            "painted yellow road lane markings, not a raised speed bump",
            "double yellow lines running forward along the road",
            "thin yellow lane paint on a normal road surface",
        ],
        "CROSSWALK": [
            "white zebra crossing stripes painted on a flat road",
            "a pedestrian crosswalk with white painted road stripes",
            "flat white crosswalk markings across the asphalt, not a raised bump",
        ],
        "YELLOW_VEHICLE": [
            "a yellow taxi or yellow car driving on the road",
            "a yellow vehicle in front of the camera",
            "yellow paint on a car body, not a speed bump",
        ],
        "ROAD_SURFACE_OBJECT": [
            "a round manhole cover on flat asphalt",
            "dark shadows and patches on a flat road surface",
            "road texture, stains, and shadows without a raised speed bump",
        ],
        "STOP_SIGN": [
        ...
        "SPEED_BUMP": [
            "a raised black and yellow striped speed bump physically blocking the lane in the foreground",
            "a rubber speed bump with alternating black and yellow rectangular blocks across the entire road",
            "a long thin raised black yellow segmented road hump perpendicular to the direction of travel",
            "a foreground traffic calming bump protruding above the asphalt across the lane",
        ],
    }

    STOP_LABELS = {"STOP_SIGN", "RED_LIGHT"}
    DEFAULT_SPEED_SCALES = {"SPEED_BUMP": 1.0}
    ...
    def __init__(self, cfg):
        ...
        self.threshold = float(getattr(cfg, "CLIP_SIGN_THRESHOLD", 0.65))
        self.margin_over_normal = float(getattr(cfg, "CLIP_SIGN_MARGIN_OVER_NORMAL", 0.15))
        self.label_thresholds = {"SPEED_BUMP": 0.50}
        self.label_margins_over_normal = {"SPEED_BUMP": -1.0}
        self.label_margins_over_runner_up = {"SPEED_BUMP": -1.0}
        self.speed_bump_logit_margin_threshold = float(getattr(cfg, "CLIP_SIGN_SPEED_BUMP_LOGIT_MARGIN_THRESHOLD", 4.2963))
        self.speed_bump_bottom_crop_enabled = bool(getattr(cfg, "CLIP_SIGN_SPEED_BUMP_BOTTOM_CROP_ENABLED", True))
        self.speed_bump_bottom_crop_frac = float(getattr(cfg, "CLIP_SIGN_SPEED_BUMP_BOTTOM_CROP_FRAC", 0.25))
        ...
        self.model_name = str(getattr(cfg, "CLIP_SIGN_MODEL", "ViT-B-32"))
    ...
    def _bottom_crop(self, imgs: torch.Tensor) -> torch.Tensor:
        frac = min(max(float(self.speed_bump_bottom_crop_frac), 0.05), 1.0)
        if imgs.dim() != 4:
            return imgs
        if imgs.shape[-1] == 3:
            h = imgs.shape[1]
            crop_h = max(int(round(h * frac)), 1)
            return imgs[:, h - crop_h:, :, :]
        h = imgs.shape[-2]
        crop_h = max(int(round(h * frac)), 1)
        return imgs[:, :, h - crop_h:, :]

    def _encode_image_logits(self, imgs: torch.Tensor, device: torch.device) -> torch.Tensor:
        imgs = self._preprocess(imgs, device)
        if self.backend == "open_clip":
        ...
        return self.temperature * image_feat @ self.text_features.to(device).T
    ...
    def infer(self, rgb_seq: torch.Tensor, device: torch.device):
        ...
        curr = rgb_seq[:, -1]
        with torch.no_grad():
            if self.speed_bump_bottom_crop_enabled and self.speed_bump_bottom_crop_frac > 0:
                decision_logits = self._encode_image_logits(self._bottom_crop(curr), device)
            else:
                decision_logits = self._encode_image_logits(curr, device)
            probs = decision_logits.softmax(dim=-1)
            best_prob, best_idx = probs.max(dim=-1)

        normal_idx = self.labels.index("NORMAL")
        speed_idx = self.labels.index("SPEED_BUMP") if "SPEED_BUMP" in self.labels else None
        ...
        actions = []
        speed_bump_score = (
            decision_logits[:, speed_idx] - decision_logits[:, normal_idx]
            if speed_idx is not None
            else torch.full((probs.shape[0],), float("-inf"), device=device, dtype=decision_logits.dtype)
        )
        for i, idx in enumerate(best_idx.tolist()):
            label = "NORMAL"
            label_prob = normal_prob[i]
            if speed_idx is not None:
                if bool(speed_bump_score[i] >= self.speed_bump_logit_margin_threshold):
                    label = "SPEED_BUMP"
                    label_prob = probs[i, speed_idx]
            labels.append(label)
            actions.append("NONE")
            if label in self.stop_labels or label in self.speed_scales:
                threshold = self.label_thresholds.get(label, self.threshold)
                normal_margin = self.label_margins_over_normal.get(label, self.margin_over_normal)
                runner_up_margin = self.label_margins_over_runner_up.get(label, 0.0)
                runner_up_prob = probs[i].topk(2).values[-1]
                confident = (
                    speed_bump_score[i] >= self.speed_bump_logit_margin_threshold
                    if label == "SPEED_BUMP"
                    else label_prob >= threshold
                )
                above_normal = label_prob >= normal_prob[i] + normal_margin
                above_runner_up = label_prob >= runner_up_prob + runner_up_margin
                if bool(confident and above_normal and above_runner_up):
        ...
        return {
            ...
            "best_idx": best_idx,
            "best_prob": best_prob,
            "speed_bump_score": speed_bump_score,
...
class SharpTurnLateralAccelerationLimiter:
    """
    Inference-time trajectory post-process using only predicted waypoints.

    It is intentionally not an nn.Module so old checkpoints can still be
    loaded strictly. It detects sharp turns from the predicted path, then
    scales forward displacement by the golf-cart/train-car turn-radius ratio
    and applies a bicycle-model forward cap.
    """
    ...
    def __init__(self, cfg):
        ...
        self.enabled = bool(getattr(
            cfg,
            "SHARP_TURN_LATACC_LIMIT_ENABLED",
            getattr(cfg, "SHARP_TURN_FORWARD_LIMIT_ENABLED", getattr(cfg, "SHARP_TURN_BOOST_ENABLED", True)),   # 約束
        ))
        self.apply_during_train = bool(getattr(cfg, "SHARP_TURN_LATACC_LIMIT_DURING_TRAIN", False))
        self.lateral_trigger_m = float(getattr(cfg, "SHARP_TURN_LATERAL_TRIGGER_M", 1.5))
        self.heading_trigger_deg = float(getattr(cfg, "SHARP_TURN_HEADING_TRIGGER_DEG", 120.0))
        self.min_forward_m = float(getattr(cfg, "SHARP_TURN_MIN_FORWARD_M", 1.5))
        self.dt = float(getattr(cfg, "SHARP_TURN_LATACC_DT", getattr(cfg, "SAMPLE_DT", 0.5)))
        self.max_lat_acc_mps2 = float(getattr(cfg, "SHARP_TURN_MAX_LAT_ACCEL_MPS2", 10.6))
        self.curvature_eps = float(getattr(cfg, "SHARP_TURN_CURVATURE_EPS", 1e-3))
        self.min_speed_ratio = float(getattr(cfg, "SHARP_TURN_MIN_SPEED_RATIO", 0.5))
        self.wheelbase_m = float(getattr(cfg, "SHARP_TURN_WHEELBASE_M", getattr(cfg, "GOLF_CART_WHEELBASE_M", 1.66)))
        self.max_steer_deg = float(getattr(cfg, "SHARP_TURN_MAX_STEER_DEG", 35.0))
        max_steer_rad = max(abs(self.max_steer_deg) * torch.pi / 180.0, 1e-3)
        self.min_turn_radius_m = float(self.wheelbase_m / torch.tan(torch.tensor(max_steer_rad)).item())
        self.train_wheelbase_m = float(getattr(cfg, "SHARP_TURN_TRAIN_WHEELBASE_M", 2.6))
        self.train_max_steer_deg = float(getattr(cfg, "SHARP_TURN_TRAIN_MAX_STEER_DEG", self.max_steer_deg))
        train_steer_rad = max(abs(self.train_max_steer_deg) * torch.pi / 180.0, 1e-3)
        self.train_turn_radius_m = float(self.train_wheelbase_m / torch.tan(torch.tensor(train_steer_rad)).item())
        raw_radius_ratio = self.min_turn_radius_m / max(self.train_turn_radius_m, 1e-6)
        self.radius_ratio = float(min(max(raw_radius_ratio, 0.0), 1.0))
        self.forward_ratio_min = float(getattr(cfg, "SHARP_TURN_FORWARD_RATIO_MIN", max(self.radius_ratio, 0.68)))
        self.forward_ratio_max = float(getattr(cfg, "SHARP_TURN_FORWARD_RATIO_MAX", 0.90))
        if self.forward_ratio_max < self.forward_ratio_min:
            self.forward_ratio_max = self.forward_ratio_min
        self.curvature_low = float(getattr(cfg, "SHARP_TURN_RATIO_CURVATURE_LOW", 0.15))
        self.curvature_high = float(getattr(cfg, "SHARP_TURN_RATIO_CURVATURE_HIGH", 0.75))
        self.turn_radius_scale = float(getattr(cfg, "SHARP_TURN_RADIUS_SCALE", 4.0))
        self.effective_turn_radius_m = float(getattr(
            cfg, "SHARP_TURN_EFFECTIVE_RADIUS_M", self.min_turn_radius_m * self.turn_radius_scale
        ))
        self.forward_cap_min_m = float(getattr(cfg, "SHARP_TURN_FORWARD_CAP_MIN_M", 3.0))
        self.forward_cap_max_m = float(getattr(
            cfg, "SHARP_TURN_FORWARD_CAP_MAX_M", getattr(cfg, "SHARP_TURN_FORWARD_LIMIT_M", 7.0)
        ))
    ...
    def apply(self, traj: torch.Tensor, commands: Optional[List[str]], training: bool = False):
        ...
        heading = torch.atan2(delta[..., 0], delta[..., 1])
        prev_heading = torch.cat([torch.zeros(b, 1, device=device, dtype=dtype), heading[:, :-1]], dim=1)
        d_heading = self._wrap_pi(heading - prev_heading)
        curvature = d_heading.abs() / seg_len.clamp(min=1e-4)
        speed = seg_len / max(self.dt, 1e-6)
        lat_acc = speed.pow(2) * curvature

        max_heading = heading.abs().amax(dim=1)
        max_curvature = curvature.amax(dim=1)
        max_lat_acc = lat_acc.amax(dim=1)
        ...
        accel_turn = max_lat_acc > self.max_lat_acc_mps2
        shape_turn = lateral_turn | heading_turn
        traj_mask = enough_forward & (accel_turn | shape_turn)

        lateral_abs = lateral.abs()
        radius = torch.tensor(self.effective_turn_radius_m, device=device, dtype=dtype)
        x_for_arc = lateral_abs.clamp(max=max(2.0 * self.effective_turn_radius_m - 1e-3, 1e-3))
        arc_forward_cap = torch.sqrt((2.0 * radius * x_for_arc - x_for_arc.pow(2)).clamp(min=0.0))
        arc_forward_cap = arc_forward_cap.clamp(min=self.forward_cap_min_m, max=self.forward_cap_max_m)
        curve_alpha = ((max_curvature - self.curvature_low) / max(self.curvature_high - self.curvature_low, 1e-6)).clamp(0.0, 1.0)
        radius_ratio = self.forward_ratio_max - curve_alpha * (self.forward_ratio_max - self.forward_ratio_min)
        # Use a curvature-dependent golf-cart/train-car ratio on sharp turns.
        # Keep the bicycle-model cap values for logging, but do not apply them here.
        cap_ratio = torch.ones(b, device=device, dtype=dtype)
        forward_ratio = torch.where(traj_mask, radius_ratio, torch.ones_like(radius_ratio))
        resampled_forward = forward * forward_ratio
        forward_cap_mask = torch.zeros_like(traj_mask)
        apply_mask = traj_mask & (forward_ratio < 1.0 - 1e-4)
        speed_ratio = forward_ratio.view(b, 1).expand_as(speed)

        if not bool(apply_mask.any()):
            info = {
                "apply_mask": apply_mask,
                "sharp_turn_mask": traj_mask,
                "command_mask": command_mask,
                "traj_mask": traj_mask,
                "enough_forward": enough_forward,
                "lateral_turn": lateral_turn,
                "heading_turn": heading_turn,
                "latacc_turn": accel_turn,
                "speed_limited": apply_mask,
                "forward_limited": forward_cap_mask,
                "forward_ratio": forward_ratio.detach(),
                "speed_ratio_min": speed_ratio.amin(dim=1).detach(),
                "max_lat_acc_mps2": max_lat_acc.detach(),
                "lat_acc_limit_mps2": torch.full((b,), self.max_lat_acc_mps2, device=device, dtype=dtype),
                "max_curvature": max_curvature.detach(),
                "forward_limit_m": arc_forward_cap.detach(),
                "turn_radius_m": torch.full((b,), self.effective_turn_radius_m, device=device, dtype=dtype),
                "min_turn_radius_m": torch.full((b,), self.min_turn_radius_m, device=device, dtype=dtype),
                "train_turn_radius_m": torch.full((b,), self.train_turn_radius_m, device=device, dtype=dtype),
                "radius_ratio": radius_ratio.detach(),
                "raw_radius_ratio": torch.full((b,), self.radius_ratio, device=device, dtype=dtype),
                "forward_ratio_min": torch.full((b,), self.forward_ratio_min, device=device, dtype=dtype),
                "forward_ratio_max": torch.full((b,), self.forward_ratio_max, device=device, dtype=dtype),
                "curve_alpha": curve_alpha.detach(),
                "wheelbase_m": torch.full((b,), self.wheelbase_m, device=device, dtype=dtype),
                "max_steer_deg": torch.full((b,), self.max_steer_deg, device=device, dtype=dtype),
                "cap_ratio": cap_ratio.detach(),
                "lateral": lateral.detach(),
                "forward": forward.detach(),
                "forward_after": resampled_forward.detach(),
                "max_heading_deg": torch.rad2deg(max_heading.detach()),
            }
            return traj, info

        adjusted = traj.clone()
        adjusted_y = xy[..., 1:2] * forward_ratio.view(b, 1, 1)
        adjusted[..., 1:2] = torch.where(apply_mask.view(b, 1, 1), adjusted_y, adjusted[..., 1:2])
...
class FastTrajectoryPlanner(nn.Module):
    ...
    def make_coarse_baseline(self, ego_seq: torch.Tensor) -> torch.Tensor:
        ...
        if t_rf >= 2:
            ...
            per_step_dxy_m = dxy_m / intervals
            step_len = torch.norm(per_step_dxy_m, dim=-1).mean(dim=-1).clamp(max=20.0)
        else:
            dxy_m = ego_seq[:, -1, 0:2] * self.EGO_SCALE_M
...
class VLM_STP3_Gen(nn.Module):
    ...
    @staticmethod
    def _csv_metadata_value(values, index: int) -> str:
        if values is None:
            return ""
        if torch.is_tensor(values):
            if values.ndim == 0:
                value = values.detach().cpu().item()
            elif index < values.shape[0]:
    ...
    def _write_clip_sign_csv(
        ...
        speed_bump_score = info.get("speed_bump_score")
        speed_bump_score = speed_bump_score.detach().float().cpu() if torch.is_tensor(speed_bump_score) else None
        ...
        with path.open(mode, newline="") as f:
            ...
            for i, pred_label in enumerate(pred_labels):
                filename = self._csv_metadata_value(filenames, i)
                row = {
                    "time": f"{time.time():.6f}",
                    "step": self._clip_sign_csv_step,
                    "batch_index": i,
                    "timestamp_us": self._csv_metadata_value(timestamps, i),
                    "filename": filename,
                    "img_timestep": pathlib.Path(filename).stem if filename else "",
                    "command": commands[i] if commands is not None and i < len(commands) else "",
                    "pred_label": pred_label,
                    "action": actions[i],
                    "best_prob": f"{float(best_prob[i]):.6f}",
                    "speed_bump_score": f"{float(speed_bump_score[i]):.6f}" if speed_bump_score is not None else "",
    ...
    def _apply_clip_sign_gate(
        ...
        if self.training and not use_during_train:
            return pred
        if not bool(getattr(self.cfg, "CLIP_SIGN_GATE_ENABLED", False)):
            return pred
        if self._last_rgb_seq is None:
            return pred

        timing_enabled = bool(getattr(self.cfg, "CLIP_SIGN_TIMING_PRINT_ENABLED", True))
        info = None
        clip_error = None
        if timing_enabled:
            self._sync_device_for_timing(pred.device)
        ...
        try:
            ...
            if timing_enabled:
                self._sync_device_for_timing(pred.device)
                elapsed_ms = (time.perf_counter() - clip_start) * 1000.0
                status = f"error={type(clip_error).__name__}" if clip_error is not None else ("ok" if info is not None else "skipped")
                print(f"[CLIP_SIGN_GATE] enabled elapsed={elapsed_ms:.3f} ms batch={pred.shape[0]} status={status}")
        ...
        if not self.training:
            self._write_clip_sign_csv(info, commands, timestamps=timestamps, filenames=filenames)
    ...
    def _write_sharp_turn_csv(self, info, commands: Optional[List[str]] = None) -> None:
        ...
        latacc_turn = info["latacc_turn"].detach().cpu()
        ...
        forward_after = info["forward_after"].detach().float().cpu()
        cap_ratio = info["cap_ratio"].detach().float().cpu()
        speed_ratio_min = info["speed_ratio_min"].detach().float().cpu()
        max_lat_acc = info["max_lat_acc_mps2"].detach().float().cpu()
        lat_acc_limit = info["lat_acc_limit_mps2"].detach().float().cpu()
        max_curvature = info["max_curvature"].detach().float().cpu()
        turn_radius = info["turn_radius_m"].detach().float().cpu()
        min_turn_radius = info["min_turn_radius_m"].detach().float().cpu()
        train_turn_radius = info["train_turn_radius_m"].detach().float().cpu()
        radius_ratio = info["radius_ratio"].detach().float().cpu()
        raw_radius_ratio = info["raw_radius_ratio"].detach().float().cpu()
        forward_ratio_min = info["forward_ratio_min"].detach().float().cpu()
        forward_ratio_max = info["forward_ratio_max"].detach().float().cpu()
        curve_alpha = info["curve_alpha"].detach().float().cpu()
        wheelbase = info["wheelbase_m"].detach().float().cpu()
        max_steer = info["max_steer_deg"].detach().float().cpu()
        ...
        with path.open("a", newline="") as f:
            ...
            for i in range(apply_mask.numel()):
                writer.writerow({
                    ...
                    "latacc_trigger": int(bool(latacc_turn[i])),
                    ...
                    "forward_after_m": f"{float(forward_after[i]):.6f}",
                    "cap_ratio": f"{float(cap_ratio[i]):.6f}",
                    "speed_ratio_min": f"{float(speed_ratio_min[i]):.6f}",
                    "max_lat_acc_mps2": f"{float(max_lat_acc[i]):.6f}",
                    "lat_acc_limit_mps2": f"{float(lat_acc_limit[i]):.6f}",
                    "max_curvature": f"{float(max_curvature[i]):.6f}",
                    "turn_radius_m": f"{float(turn_radius[i]):.6f}",
                    "min_turn_radius_m": f"{float(min_turn_radius[i]):.6f}",
                    "train_turn_radius_m": f"{float(train_turn_radius[i]):.6f}",
                    "radius_ratio": f"{float(radius_ratio[i]):.6f}",
                    "raw_radius_ratio": f"{float(raw_radius_ratio[i]):.6f}",
                    "forward_ratio_min": f"{float(forward_ratio_min[i]):.6f}",
                    "forward_ratio_max": f"{float(forward_ratio_max[i]):.6f}",
                    "curve_alpha": f"{float(curve_alpha[i]):.6f}",
                    "wheelbase_m": f"{float(wheelbase[i]):.6f}",
                    "max_steer_deg": f"{float(max_steer[i]):.6f}",
    ...
    def planning(
        ...
        *,
        ...
        clip_sign_timestamps=None,
        clip_sign_filenames=None,
    ):
        ...
        final_traj = self._apply_clip_sign_gate(
            pred,
            commands,
            timestamps=clip_sign_timestamps,
            filenames=clip_sign_filenames,
        )
