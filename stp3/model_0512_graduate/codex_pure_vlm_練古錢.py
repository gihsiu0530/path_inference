...
class ClipTrafficSignGate:
    ...
    DEFAULT_PROMPTS: Dict[str, List[str]] = {
        "NORMAL": [
            ...
            # "a flat road surface with no raised speed bump",
            # "a road ahead with lane markings but no speed bump",
            "a zebra crossing painted on a flat road with no speed bump",                 # 標準的白色車道線且路面淨空
            "white lane lines on a safe road",
            "yellow lane lines on a safe road",
        ],
        "STOP_SIGN": [
        ...
        # "NO_SPEED_BUMP": [
        #     "a flat road surface without a speed bump",
        #     "a normal road with no raised bump across the lane",
        #     "lane markings on a flat road surface",
        #     "road shadows and pavement markings but no speed bump",
        #     "a zebra crossing painted on the road, not a raised speed bump",
        #     "a pedestrian crosswalk with white stripes on a flat road",
        # ],
        "SPEED_BUMP": [
            "a raised speed bump directly ahead across the lane",
            # "a speed bump spanning across the road surface",
            # "a transverse raised bump across the driving lane",
            # "a speed hump painted on the road across the lane",
        ],
    }

    STOP_LABELS = {"STOP_SIGN", "RED_LIGHT"}
    DEFAULT_SPEED_SCALES = {"SPEED_BUMP": 0.75}
    ...
    def __init__(self, cfg):
        ...
        self.label_margins_over_normal = {"SPEED_BUMP": 0.15}
        self.label_margins_over_runner_up = {"SPEED_BUMP": 0.08}
        self.speed_bump_bottom_crop_enabled = bool(getattr(cfg, "CLIP_SIGN_SPEED_BUMP_BOTTOM_CROP_ENABLED", True))
        self.speed_bump_bottom_crop_frac = float(getattr(cfg, "CLIP_SIGN_SPEED_BUMP_BOTTOM_CROP_FRAC", 0.40))
    ...
    def infer(self, rgb_seq: torch.Tensor, device: torch.device):
        ...
        curr = rgb_seq[:, -1]
        with torch.no_grad():
            logits = self._encode_image_logits(curr, device)
            decision_logits = logits.clone()
            if self.speed_bump_bottom_crop_enabled and "SPEED_BUMP" in self.labels:
                crop_logits = self._encode_image_logits(self._bottom_crop(curr), device)
                speed_idx = self.labels.index("SPEED_BUMP")
                decision_logits[:, speed_idx] = torch.maximum(logits[:, speed_idx], crop_logits[:, speed_idx])
                if "NO_SPEED_BUMP" in self.labels:
                    no_speed_idx = self.labels.index("NO_SPEED_BUMP")
                    decision_logits[:, no_speed_idx] = torch.maximum(logits[:, no_speed_idx], crop_logits[:, no_speed_idx])
        ...
        for i, idx in enumerate(best_idx.tolist()):
            ...
            if label in self.stop_labels or label in self.speed_scales:
                ...
                confident = best_prob[i] >= threshold
                above_normal = best_prob[i] >= normal_prob[i] + normal_margin
                above_runner_up = best_prob[i] >= runner_up_prob + runner_up_margin
...
class SharpTurnLateralAccelerationLimiter:
    ...
    def __init__(self, cfg):
        ...
        self.enabled = bool(getattr(
            ...
            getattr(cfg, "SHARP_TURN_FORWARD_LIMIT_ENABLED", getattr(cfg, "SHARP_TURN_BOOST_ENABLED", True)),
        ))
